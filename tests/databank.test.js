...
describe("whether the hub is wired up at all", () => {
  it("does nothing, quietly, when nobody has connected one", async () => {
    ...
    const { configured, sendToDataBank } = await load({ url: "", key: "" });
    globalThis.fetch = () => {
      throw new Error("must not reach the network");
    };

    assert.equal(configured(), false);
    const seen = await sendToDataBank({ kind: "REGISTRATION", payload: {} });
...
describe("what reaches the hub", () => {
  it("presents the key in the header the hub reads", async () => {
    ...
    await sendToDataBank({ kind: "REGISTRATION", payload: { name: "A" } });
  ...
  it("does not double the slash when the address has a trailing one", async () => {
    ...
    const { sendToDataBank } = await load({ url: `${URL}/` });
    hubAccepts();
  ...
  it("stamps every payload with which product sent it and when", async () => {
    ...
    await sendToDataBank({ kind: "RESULT_FIGURES", payload: { unitCode: "25/07/04/019" } });
  ...
  it("carries media hashes so a figure can be paired with its photograph", async () => {
    ...
    await sendToDataBank({ kind: "RESULT_FIGURES", payload: {}, mediaHashes: ["abc123"] });
...
describe("when the hub is having a bad night", () => {
  ...
  it("returns why rather than throwing, when it is too slow", async () => {
    ...
    const seen = await sendToDataBank({ kind: "RESULT_SHEET", payload: {} });
    assert.equal(seen.sent, false);
  ...
  it("never rejects out of the fire-and-forget form, whatever happens", async () => {
    /* ── THE TEST THIS FILE EXISTS FOR ───────────────────────────────────
       `forwardToDataBank` is called from server actions on a user's path. An
    ...
    const { forwardToDataBank } = await load();
    globalThis.fetch = async () => {
    ...
    assert.equal(forwardToDataBank({ kind: "REGISTRATION", payload: {} }), undefined);
  ...
  it("gives the caller nothing to await, so it cannot be put on the path", async () => {
    /* Shape as documentation: `forwardToDataBank` returns undefined on
    ...
    const { forwardToDataBank } = await load();
    hubAccepts();
    assert.equal(forwardToDataBank({ kind: "MESSAGE", payload: {} }), undefined);
  });
});
