...
function paperFor(_race) {
  return NATIONAL.map((id) => {
    ...
    /* Optional on a sheet: only the presidential four are printed on every
       result sheet in the country, so only they may be treated as "must be
       here" by a reader. See lib/sheet-vision.js. */
    const core = parties.some((party) => party.id === id);
    return core ? canonical : { ...canonical, optional: true };
  });
}
