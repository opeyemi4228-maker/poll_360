...
/**
 * The stay-at-home pool, state by state.
 *
 * ── THE LARGEST TARGET LIST IN NIGERIAN POLITICS ───────────────────────────
 * Sixty-five million registered voters did not vote in 2023, against
 * twenty-four million who did. Every campaign plans around persuading people
 * away from another party; the arithmetic says the bigger prize by a factor
 * of nearly three is the register that stayed home, and it is a prize nobody
 * has to be argued out of a rival's hands to win.
 *
 * Sorted by the size of that pool rather than by turnout rate, because a
 * campaign spends per person and not per percentage point: a big state at 30%
 * has more people to reach than a small one at 15%.
 */
export function stayAtHomeByState() {
  const latest = ELECTIONS[ELECTIONS.length - 1];
  const cast = new Map(latest.rows.map((row) => [row.code, row.total]));

  return states2023
    .map((state) => {
      const voted = cast.get(state.code) ?? 0;
      return {
        code: state.code,
        name: state.name,
        registered: state.registered,
        cast: voted,
        stayed: Math.max(0, state.registered - voted),
        turnout: state.registered ? (voted / state.registered) * 100 : 0,
      };
    })
...
/**
 * The states that changed hands at a given election.
 *
 * Only against the previous *recorded* election, which with 2007 and 2011
 * absent means a change between 2003 and 2015 is reported as exactly that.
 * A campaign asking "where is this winnable" wants the places that have
 * demonstrably moved, and a place that moved at an unknown point between two
 * elections has still demonstrably moved.
 */
export function flippedAt(year) {
  return stateHistories()
    .filter((state) => state.changes.some((change) => change.to === year))
    .map((state) => {
      const change = state.changes.find((item) => item.to === year);
      const before = state.run.find((step) => step.year === change.from);
      const after = state.run.find((step) => step.year === change.to);
      return {
        code: state.code,
        name: state.name,
        zone: state.zone,
        from: before?.winner ?? null,
        to: after?.winner ?? null,
        since: change.from,
        margin: after?.margin ?? 0,
      };
    })
