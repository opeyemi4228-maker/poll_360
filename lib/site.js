 ...
 *
 * ── AN EMPTY SETTING USED TO TAKE THE WHOLE BUILD DOWN ─────────────────────
 * `NEXT_PUBLIC_SITE_URL` left blank in a hosting dashboard is an easy thing to
 * do and looks like nothing. It is not nothing: `??` fills in a value that is
 * missing and not one that is empty, so the empty string reached
 * `new URL(site.url)` in app/layout.js while Next was collecting metadata, and
 * every page in the build failed at once with "Invalid URL".
 *
 * So each candidate is tried in turn and anything that is not a full address —
 * blank, whitespace, a typo — is passed over rather than trusted: the setting
 * first, then the address the platform gives a build of its own accord, then
 * this product's own domain. A site that cannot be told where it lives should
 * still deploy and still serve; only its canonical links are affected.
 */
export function siteAddress(...candidates) {
  for (const candidate of candidates) {
    const value = String(candidate ?? "").trim();
    if (!value) continue;
    /* Hosting platforms hand out a bare hostname, people paste a full one. */
