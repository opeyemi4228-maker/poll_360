 ...
 * `logo`   true where public/parties/<id lowercased>.png exists.
 *
 * ── WHY THE LOGO IS A FLAG HERE AND NOT A PATH DERIVED FROM THE ID ────────
 * The portraits in public/people work the other way — a path is built from
 * the name and a missing file simply does not draw — and that is right for a
 * screen, where a failed request costs nothing anybody sees.
 *
 * These are drawn into result cards that get rendered to a file on the server
 * for broadcast, and an `onError` fallback does not run there. A party with no
 * file would export a broken image into a graphic going out under the
 * organisation's name. So the register states which files exist, and nothing
 * is ever requested that is not known to be there.
 *
 * Adding one is two steps, and public/parties/README.md carries both.
 */
const REGISTER = [
  ...
  { id: "APC", name: "All Progressives Congress", token: "var(--color-apc)", logo: true },
  { id: "PDP", name: "Peoples Democratic Party", token: "var(--color-pdp)", logo: true },
  /* LP carries a hatch as well as a colour: its red and PDP's green are the
     same tone to a protanope. See lib/party-pattern.js. */
  { id: "LP", name: "Labour Party", token: "var(--color-lp)", pattern: "diagonal", logo: true },
  ...
  { id: "ADC", name: "African Democratic Congress", token: "var(--color-adc)", logo: true },
...
/**
 * The party's own mark, where this deployment holds one.
 *
 * Null rather than a guessed path: a caller that gets null draws the colour
 * swatch instead, which is a finished state and not a placeholder — see
 * components/dash/PartyMark.jsx and public/parties/README.md.
 */
export function partyLogo(id) {
  const found = partyById(id);
  return found?.logo ? `/parties/${found.id.toLowerCase()}.png` : null;
}
 ...
 *
 * ── NO PARTY IS PRINTED ON EVERY SHEET ────────────────────────────────────
 * This used to mark the presidential four as required, on the reasonable-
 * sounding theory that APC, PDP, LP and NNPP appear on every result sheet in
 * Nigeria. The Osun 2026 governorship paper carries neither PDP nor LP.
 *
 * A reader holding that theory reads that sheet perfectly and then reports
 * "could not read PDP, LP", which marks the reading unusable and silently
 * cancels the comparison against what the agent typed — losing the
 * corroboration on the very form this product was built around. Which parties
 * are on a paper is a property of the paper, and the only thing a reader can
 * say about a row is whether it found one.
 */
export function scanList() {
  return REGISTER.map((party) => ({ token: OTHER_TOKEN, ...party }));
}
