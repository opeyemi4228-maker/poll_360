...
export function matchSheet(parsed, typed) {
  ...
  /* A party the reader could not make out. parseSheet stores 0 for these so
     the array keeps its shape, which makes `missing` the only way to tell an
     unread figure from a genuine nil. */
  const unread = new Set(parsed.missing ?? []);
