...
function mergeReadings(first, second) {
  ...
  const votes = parties.map((party, index) => {
    const missing = first.missing.includes(party.id);
    if (!missing) return first.votes[index];
    /* Only if the second pass actually read it. Its own zero-for-unread
       placeholder is not a reading. */
    if (second.missing.includes(party.id)) return 0;
    return second.votes[index] ?? 0;
  });

  const missing = [];
  for (const [index, party] of parties.entries()) {
    if (first.missing.includes(party.id) && second.missing.includes(party.id)) {
      missing.push(party.id);
    } else if (votes[index] === null || votes[index] === undefined) {
      missing.push(party.id);
    }
  }
...
export function figuresForBallot(parsed, race) {
  const votes = Object.fromEntries(
    ballotFor(race).map((party, index) => {
      if (party.id === "OTH") {
      ...
      return [party.id, parsed?.votes?.[index] ?? null];
    })
  );
