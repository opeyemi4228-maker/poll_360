...
export function divergenceOf(ours, declared, expected = new Map()) {
  ...
  /* ── "DIFFERING" MUST NOT COUNT PLACES WE SIMPLY HAVE NOT REACHED ────────
     UNMATCHED means a figure was declared and we hold no return there. That is
     a statement about our own coverage, not a disagreement between two counts,
     and folding it in here would put every place our agents have not got to
     yet into the number a room reads as "where the figures differ" — which on
     the first night of collation is most of the country. It is reported on its
     own, as `unmatched`. */
  const places = new Set(
    flags
      .filter((flag) => flag.severity !== "UNMATCHED")
      .map((flag) => `${flag.level}:${flag.key}`)
  );
