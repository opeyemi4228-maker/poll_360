...
const asDate = (value) => (value ? new Date(`${value}Z`) : null);
...
export async function health() {
      ...
      const row = await db
        .prepare(
          `SELECT (SELECT COUNT(*)::int FROM users)                                  AS users,
                  ...
                  (SELECT COALESCE(SUM(OCTET_LENGTH(bytes)), 0) FROM media)          AS media_bytes,
  ...
  return { reachable, latency, complaint, counts, freshness };
}
