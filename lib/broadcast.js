...
/**
 * What the public wire carries.
 *
 * ══════════════════════════════════════════════════════════════════════════
 *  THE WIRE IS AIR, SO THE WIRE IS WHAT A PERSON PUT ON AIR
 *
 *  /live/p/[project] is a transmitter with an address. Everything the rest of
 *  this file says about clearance applies to it exactly as it applies to a
 *  strap on the picture: nothing reaches it that an editor has not passed.
 *
 *  That leaves the question of *which* on-air things belong at a public
 *  address, and the answer is the ones written for a reader rather than for a
 *  screen. A ticker is a crawl along the bottom of a picture; a lower third is
 *  keyed over a face; a clearance is an instruction to a gallery. None of them
 *  are sentences somebody lands on from a link and understands. These four
 *  are:
 *
 *    SOCIAL     a written update with its own card, its place and its minute
 *    BANNER     the breaking strap — the loudest thing the desk does
 *    VIDEO      a programme that is open right now
 *    PROGRAMME  a slot that is running right now
 *
 *  The list lives here, beside the kinds themselves, because it was written
 *  three times — the wire, the post's own page and the picture route — and
 *  three copies of a rule about what is public is three chances to widen one
 *  of them by accident.
 * ══════════════════════════════════════════════════════════════════════════
 */
export const PUBLIC_KINDS = ["SOCIAL", "BANNER", "VIDEO", "PROGRAMME"];

/** Is this item something the public wire may show, right now? */
export const isPublic = (item) =>
  Boolean(item) && item.state === "ON_AIR" && PUBLIC_KINDS.includes(item.kind);

/**
 * Does this item have a card of its own?
 *
 * Only a social post does. Its payload is built on the server by
 * lib/post-payload.js and holds the frozen figures lib/cards.jsx draws from;
 * a strap carries three words and a programme carries stream addresses, and
 * handing either to the card renderer produces a beautifully typeset blank.
 * So the picture route serves these and nothing else, and the wire draws the
 * rest as type.
 */
export const hasCard = (item) => item?.kind === "SOCIAL" && item?.payload != null;

/** A programme that is open right now, if one is. */
export const programmeOnAir = (items = []) =>
  items.find((item) => (item.kind === "VIDEO" || item.kind === "PROGRAMME") && item.state === "ON_AIR") ?? null;
...
/**
 * The platforms, and how a post reaches each one.
 *
 * ── `route` IS A FACT ABOUT THIS PRODUCT, NOT ABOUT THIS DEPLOYMENT ────────
 * "direct" means lib/publish.js has an adapter that posts there itself;
 * "relay" means it can only get there through the organisation's own
 * automation, or a person. Whether a direct platform is actually *connected*
 * tonight is a different question — it depends on credentials and on the
 * platform answering — and it is asked live by `checkChannels` in
 * app/broadcast/actions.js, never assumed from this table. A green light read
 * off a constant is how a desk comes to believe a post went out.
 */
export const PLATFORMS = [
  { id: "facebook", label: "Facebook", shape: "square", carries: "Image, video, text", route: "direct" },
  { id: "instagram", label: "Instagram", shape: "square", carries: "Image, reel", route: "direct" },
  { id: "x", label: "X", shape: "wide", carries: "Image, video, text", route: "direct" },
  { id: "threads", label: "Threads", shape: "square", carries: "Image, text", route: "direct" },
  { id: "telegram", label: "Telegram", shape: "square", carries: "Image, video, text", route: "direct" },
  { id: "whatsapp", label: "WhatsApp", shape: "square", carries: "Image, text", route: "relay" },
  { id: "linkedin", label: "LinkedIn", shape: "wide", carries: "Image, text", route: "relay" },
  { id: "tiktok", label: "TikTok", shape: "story", carries: "Short video", route: "relay" },
  { id: "youtube", label: "YouTube", shape: "wide", carries: "Video, short, live", route: "relay" },
];

export const platformLabel = (id) => PLATFORMS.find((row) => row.id === id)?.label ?? id;

/** What each delivery status means, in the words the desk reads. */
export const DELIVERY = {
  SENT: { label: "Posted", tone: "good" },
  RELAYED: { label: "Handed to relay", tone: "good" },
  FAILED: { label: "Failed", tone: "alert" },
  NOT_CONNECTED: { label: "Not connected", tone: "warn" },
  BY_HAND: { label: "Post by hand", tone: "warn" },
};

/**
 * The latest delivery per platform, per post.
 *
 * @param dispatches  every attempt, oldest first
 * @returns           { [itemId]: { [platform]: attempt } }
 */
export function latestDeliveries(dispatches = []) {
  const out = {};
  for (const row of dispatches) {
    out[row.itemId] ??= {};
    out[row.itemId][row.platform] = row;
  }
  return out;
}
