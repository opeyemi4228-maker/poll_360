...
export const PHASES = [
  {
    ...
    nominal: "05:00",
    evidence: "The first coordinator to open their account today.",
  },
  {
    id: "located",
  ...
  },
  {
    ...
    nominal: "08:30",
    evidence: "The first report filed from the field.",
  },
  {
    id: "counting",
    label: "Counting at the booths",
    nominal: "14:30",
    evidence: "The first return to arrive.",
  },
  {
    id: "arriving",
...
export function nightTimeline({
  ...
  const opening = earliest([firstFiled, firstIncident, firstSeen, firstFix]);
  ...
  const slots = Array.from({ length: span }, (_, index) => {
    ...
    return {
      ...
      /* Where it came from, so the row can be read without opening it. */
      states: [
        ...new Set(filedHere.map((row) => stateOf(row.unitCode)?.name).filter(Boolean)),
      ],
    };
  });
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════ */
  const perState = byStateFor({ rows, incidents, coordinators, slots, width });
        ...
        : phase.id === "counting" && rows.length
          ...
          : phase.id === "verifying"
            ...
            : phase.id === "deploy" && coordinators.length
              ...
              : phase.id === "opened" && incidents.length
 ...
 */
function byStateFor({ rows, incidents, coordinators, slots, width }) {
  const held = new Map();
  ...
  for (const state of held.values()) {
    ...
    const at = {
      deploy: min(state.seen),
      located: min(state.fixed),
      opened: min(state.reported),
      counting: min(state.filed),
    ...
    const figure = {
      ...
      located: state.fixed.length ? `${state.fixed.length} placed` : null,
      opened: state.reported.length ? plural(state.reported.length, "report") : null,
