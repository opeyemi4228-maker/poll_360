...
/**
 * How far somebody has got with typing their code, so the form can tell them
 * what is missing instead of greying out the button with no reason.
 *
 * ── THE MISTAKE THIS EXISTS FOR ────────────────────────────────────────────
 * An agent told "it starts with your polling unit number" types 33/01/01/001
 * and stops. That is not a code and must never become one, but it is the
 * commonest thing that will be typed into that box, and it deserves a sentence
 * saying six more symbols come after it.
 *
 * Returns "empty", "complete", "unit-only", "too-long", "stray" (something
 * that is not a letter, digit or separator — usually words around the code),
 * or "incomplete". For the form's wording only; sign-in still asks Data Bank.
 */
export function readTypedAgentCode(input) {
  const raw = String(input ?? "").toUpperCase().trim();
  if (!raw) return "empty";
  if (normaliseAgentCode(raw)) return "complete";

  const folded = [...raw.replace(/[\s\-_./:]+/g, "")].map((character) => FOLD[character] ?? character).join("");
  if (![...folded].every((character) => ALPHABET.includes(character))) return "stray";
  if (/^\d{9}$/.test(folded)) return "unit-only";
  if (folded.length > 9 + SECRET_LENGTH) return "too-long";
  return "incomplete";
}
