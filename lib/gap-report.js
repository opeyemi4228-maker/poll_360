 ...
 * explains worst.
 */
export async function gapReport(electionId, race, territory = null) {
  if (!electionId) return empty();

  /* ── ONE CONTEST ON BOTH SIDES OF THE COMPARISON ────────────────────────
     Both halves are read for the same position. That is the whole integrity of
     this screen: holding our presidential returns against a governorship
     declaration compares two different elections and reports the difference
     between them as a divergence, which is the exact failure this room exists
     to catch and would have been committing itself. */
  /* Both halves narrowed to the same ground, which matters here more than
     anywhere: a comparison with our district's returns on one side and the
     whole state's declaration on the other is not a divergence, it is a
     subtraction of two different places. */
  const [ours, theirs] = await Promise.all([
    results.counted(electionId, race, territory),
    declared.all(electionId, race, territory),
  ]);
  ...
  return {
    ...
    at: await declared.lastEntry(electionId, race),
  };
}
