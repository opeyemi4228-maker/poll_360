 ...
 * usable credentials, and who is asking is read from the database — held for
 * a few seconds under load — so that revoking a session or suspending an
 * account takes effect within that window rather than whenever the token
 * happens to expire. See lib/session-cache.js.
 *
...
/* A different name from `poll360_session`, so a browser can hold both at once
   ...
   The flags and the `__Host-` prefix are shared with the staff twin — see
   lib/session-cookie.js, which exists because both files used to carry a
   comment admitting that a change made in one of them would be forgotten in
   the other. */
...
export async function createCoordinatorSession(coordinatorId, { userAgent } = {}) {
  ...
  writeCookie(jar, COOKIE, token, { expires: expiresAt });
...
export const currentCoordinator = cache(async function currentCoordinator() {
  ...
  /* Held for a few seconds across requests, exactly as the staff twin is, and
     for the same reason: this runs on every request from every signed-in
     agent, and there are four thousand of them to a newsroom's four. See
     lib/session-cache.js for what the window costs. */
  const id = digest(token);
  return await recogniseWith(id, () => coordinatorSessions.find(id));
});
...
export async function destroyCoordinatorSession() {
  ...
  if (token) {
    const id = digest(token);
    forgetSession(id);
    await coordinatorSessions.destroy(id);
  }
...
export async function requireCoordinator() {
  ...
  if (!person) redirect(agentPath("/login"));
  if (!person.canFile) redirect(agentPath("/pending"));
  return person;
}
