...
export function lastResultFor({ race, territory }) {
  ...
  return {
    ...
    registered: null,
    votesOn: seat.since,
    /* ── NOT A DECLARATION DATE, AND NOT LABELLED AS ONE ──────────────────
       What we hold for these seats is when the holder took them, which for a
       National Assembly seat is the swearing-in and for a council the polling
       day. Printing either under "declared" is a small lie about a small
       thing, and the small things are what a reader calibrates trust on. The
       panel prints `heldSince` as "held since". */
    declaredOn: null,
    heldSince: seat.since,
    source: seat.source,
    /* No generic filler. The panel already says, in full, that no totals were
       published; a second sentence saying it again is a line a reader learns
       to skip, and the next line they skip is one that mattered. Only a note
       specific to this seat survives — a court that changed the result, a
       sweep that explains itself. */
    note: seat.note ?? null,
  };
}
