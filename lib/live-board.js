...
export async function liveBoard({ electionId, race, territory = null, extra = [] }) {
  /* Narrowed before the sum, not after it. A board built from every return in
     the country and then filtered is a board whose totals were correct for
     somebody else — see `within` in lib/db.js. */
  const ours = electionId ? await results.counted(electionId, race, territory) : [];

  /* ── RETURNS THIS PRODUCT NEVER SAW ─────────────────────────────────────
     Rows Data Bank holds for booths we have none for — see lib/hub-returns.js,
     which is where every rule about what may be added lives, including the one
     that stops a return being counted twice. They arrive already deduped
     against `ours`, already stripped of machine readings and of anything the
     hub's own arithmetic refused, and in the same shape as a row from
     `results.counted`. Nothing below this line knows or cares which source a
     row came from, which is the point: one definition of a board.

     Appended rather than interleaved. The order decides the event sequence and
     nothing in a live count plays back, so the only thing it costs is the
     order the ticker names booths in, and a hub row has no better claim to a
     place in that queue than the moment it was learned about. */
  const rows = extra.length ? [...ours, ...extra] : ours;
 ...
 * from Ikeja.
 */
export async function liveTree({ electionId, race, territory = null, extra = [] }) {
  const ours = electionId ? await results.counted(electionId, race, territory) : [];
  /* The same rows the board counted, so a state's total and the booths under
     it cannot tell different stories. A hub row carries no position, so it
     draws where the register puts its booth rather than where a phone was. */
  const rows = extra.length ? [...ours, ...extra] : ours;
  const ballot = ballotFor(race);
  ...
  return groupUnits(
    nameUnits(
      rows.map((row) => ({
        ...
        votes: ballot.map((party) => Number(row.votes?.[party.id] ?? 0)),
        at: row.submittedAt ?? null,
        /* Where the return was filed from, when the agent's phone had a fix.
           Carried all the way down to the polling unit so a map can draw the
           booth where it actually is rather than where a diagram put it. It
           is the only coordinate in this product that was measured, and it
           stops here rather than being averaged upward: a ward has no
           position, it has booths that each have one. */
