...
export async function liveBoard({ electionId, race, territory = null }) {
  ...
  const rows = electionId ? await results.counted(electionId, race, territory) : [];
 ...
 * from Ikeja.
 */
export async function liveTree({ electionId, race, territory = null }) {
  const rows = electionId ? await results.counted(electionId, race, territory) : [];
  const ballot = ballotFor(race);
