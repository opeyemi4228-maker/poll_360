...
async function advance({ contact, session, step, text, kind, mediaId, imageBytes }) {
  ...
  switch (step) {
    ...
    case STEPS.VOTES: {
      ...
      await whatsapp.saveSession(contact.id, STEPS.PHOTO, { ...draft, votes: withOthers });
      return {
        step: STEPS.PHOTO,
    ...
    case STEPS.PHOTO: {
      if (kind === "image" && mediaId) {
        const read = await readSheetIfPossible({ contact, draft, mediaId, imageBytes });
        await whatsapp.saveSession(contact.id, STEPS.CONFIRM, { ...draft, mediaId, readId: read?.id ?? null });
        return {
          step: STEPS.CONFIRM,
          read,
      ...
      if (said === "SKIP") {
        await whatsapp.saveSession(contact.id, STEPS.CONFIRM, draft);
    ...
    case STEPS.CONFIRM: {
      if (said === "NO" || said === "N") {
        ...
        await whatsapp.saveSession(contact.id, STEPS.ACCREDITED, { unitCode: draft.unitCode, mediaId: draft.mediaId });
        return {
          step: STEPS.ACCREDITED,
