import { whatsapp, results, units, positions, sheetReads } from "./db.js";
/* An agent identifying themselves by their code. Data Bank confirms it — the
   same answer the web form gets, see lib/databank-agents.js. */
import { looksLikeAgentCode } from "./agent-code.js";
import { confirmAgentCode } from "./databank-agents.js";
import { coordinators } from "./coordinators.js";
import { elections } from "./elections.js";
import { defaultRace } from "./races.js";
import { parties } from "./election2023.js";
import { screenReturn } from "./anomalies.js";
import { parseUnitCode } from "./units.js";
import { readSheet, visionAvailable } from "./sheet-vision.js";
import { matchSheet, matchRecord, mismatchMessage } from "./sheet-match.js";
import { STEPS } from "./whatsapp-steps.js";
/* The hub every input is also delivered to. Fire-and-forget by construction
   — see lib/databank.js. */
import { KIND, forwardToDataBank } from "./databank.js";

export { STEPS, STEP_LABEL } from "./whatsapp-steps.js";

/**
 * The Poll360 WhatsApp bot.
 *
 * ── WHY WHATSAPP AT ALL ────────────────────────────────────────────────────
 * An agent at a polling unit in Zamfara has a phone, a WhatsApp account and
 * about two bars of signal. Asking them to install an app, create an account
 * and keep a browser session alive for eleven hours is asking most of them to
 * fail. They already know how to send a message and a photograph. So the
 * product meets them there, and the dashboard is where the room reads it.
 *
 * ── IT IS A FORM, NOT A CHATBOT ────────────────────────────────────────────
 * This asks one question at a time and refuses to move on until the answer is
 * valid. That is deliberately unglamorous. A free-text assistant that guesses
 * what somebody meant by "about 200 I think" is a system that files a number
 * nobody said, and the whole product exists to make figures checkable.
 *
 * ── AN UNVERIFIED NUMBER CANNOT FILE ───────────────────────────────────────
 * Anybody can send a WhatsApp message to a published number. So a number that
 * has not been tied to a real account by an operator can talk to the bot, and
 * its answers are held as a draft that a human has to claim. Nothing reaches
 * the results table from a stranger.
 * ───────────────────────────────────────────────────────────────────────────
 */


const HELP = [
  "Poll360.",
  "",
  "Send RESULT to file the figures from your polling unit.",
  "Send PHOTO to add a picture of the result sheet.",
  "Send STATUS to see what has been filed from your unit.",
  "Send CANCEL at any point to stop and start again.",
  "",
  "Somebody on the desk reads everything sent here.",
].join("\n");

const digits = (text) => {
  const cleaned = String(text ?? "").replace(/[, ]/g, "").trim();
  return /^\d+$/.test(cleaned) ? Number(cleaned) : null;
};

const command = (text) => String(text ?? "").trim().toUpperCase();

/**
 * One inbound message in, one reply out.
 *
 * Everything is written down before a reply is composed, so a crash between
 * the two loses the reply and never the message. A missing reply is a person
 * repeating themselves; a missing message is a lost result.
 */
export async function handleInbound({
  phone,
  name = null,
  waId = null,
  text = "",
  kind = "text",
  mediaId = null,
  imageBytes = null,
  location = null,
}) {
  const contact = await whatsapp.contactFor(phone, name);

  if (contact.status === "BLOCKED") {
    /* Recorded, never answered. A blocked number that gets a reply learns it
       is blocked, and then simply uses another number. */
    await whatsapp.record({ waId, contactId: contact.id, direction: "IN", kind, body: text, mediaId, step: "BLOCKED" });
    return { contact, reply: null, step: "BLOCKED" };
  }

  const session = await whatsapp.session(contact.id);
  const step = session?.step ?? STEPS.IDLE;

  await whatsapp.record({
    waId,
    contactId: contact.id,
    direction: "IN",
    kind,
    body: location ? describePlace(location) : text,
    mediaId,
    step,
  });

  /* A position is not a step in the conversation. It arrives whenever the
     coordinator sends it, including in the middle of filing, and it must not
     disturb where they had got to. So it is banked and answered on its own,
     and the return carries on exactly where it was. */
  if (location) {
    const outcome = await recordPlace(contact, location);
    await whatsapp.record({ contactId: contact.id, direction: "OUT", body: outcome.reply, step, status: "QUEUED" });
    return { contact, ...outcome, step };
  }

  const outcome = await advance({ contact, session, step, text, kind, mediaId, imageBytes });

  if (outcome.reply) {
    await whatsapp.record({
      contactId: contact.id,
      direction: "OUT",
      body: outcome.reply,
      step: outcome.step,
      status: "QUEUED",
    });
  }

  return { contact, ...outcome };
}

async function advance({ contact, session, step, text, kind, mediaId, imageBytes }) {
  const draft = session?.draft ?? {};
  const said = command(text);

  /* Commands that work from anywhere, checked before the step machine, so
     CANCEL always cancels even halfway through a list of party figures. */
  if (said === "CANCEL" || said === "STOP") {
    await whatsapp.endSession(contact.id);
    return { step: STEPS.IDLE, reply: "Stopped. Nothing was filed. Send RESULT when you are ready to start again." };
  }

  if (said === "HELP" || said === "MENU" || said === "HI" || said === "HELLO") {
    return { step, reply: HELP };
  }

  if (said === "STATUS") {
    return { step, reply: await statusFor(contact) };
  }

  /* ══════════════════════════════════════════════════════════════════════
     THE CODE, AS A WAY IN

     ── WHY THIS IS CHECKED BEFORE THE STEP MACHINE ─────────────────────────
     An agent sends their code when they are lost. They have been asked for a
     number and cannot remember which, or the desk has told them to link their
     phone, or they are simply starting. Whatever step the conversation had
     reached, a code means "this is who I am", and it should not be read as an
     answer to "how many were accredited".

     ── AND WHY THE NUMBER IS THE SECOND FACTOR ─────────────────────────────
     Over WhatsApp the sender's number is authenticated by Meta and cannot be
     forged into this webhook — the signature check at the door sees to that.
     So a code arriving from a number is code-plus-number, and once the two are
     bound the agent never sends the code again: the number alone identifies
     them from then on, which is the only thing that works for somebody filing
     figures one line at a time on a handset.
     ══════════════════════════════════════════════════════════════════════ */
  if (looksLikeAgentCode(text)) {
    const confirmed = await confirmAgentCode(text);

    if (confirmed.state === "unavailable") {
      return {
        step,
        reply: "Codes cannot be checked right now. Nothing is wrong with yours — send it again in a few minutes.",
      };
    }

    /* A confirmed agent is kept as the row their filed results hang off. */
    const person = confirmed.state === "matched" ? await coordinators.mirrorAgent(confirmed.agent) : null;

    if (!person) {
      /* Deliberately vague, and deliberately not "no such code". A reply that
         distinguished a malformed code from an unknown one would let somebody
         with a stolen handset work out which booths have agents. */
      return {
        step,
        reply:
          "That code did not match an account. Check it against the card you were given, or ask your coordinator to issue a new one.",
      };
    }

    await whatsapp.claim(contact.id, {
      userId: null,
      unitCode: person.unitCode,
      stateCode: person.stateCode,
    });

    return {
      step: STEPS.IDLE,
      reply: person.canFile
        ? `Thank you, ${person.name.split(" ")[0]}. This number is now linked to polling unit ${person.unitCode}. You will not need the code again — send RESULT when you are ready to file.`
        : `Thank you, ${person.name.split(" ")[0]}. This number is now linked to polling unit ${person.unitCode}. Your account is still waiting to be approved, so nothing can be filed yet. Your coordinator will tell you when it is.`,
    };
  }

  if (said === "RESULT" || said === "START") {
    if (contact.unitCode) {
      await whatsapp.saveSession(contact.id, STEPS.ACCREDITED, { unitCode: contact.unitCode });
      return {
        step: STEPS.ACCREDITED,
        reply: `Filing for polling unit ${contact.unitCode}.\n\nHow many voters were accredited? Send the number only.`,
      };
    }
    await whatsapp.saveSession(contact.id, STEPS.UNIT, {});
    return { step: STEPS.UNIT, reply: "What is your polling unit code? It is printed at the top of the result sheet." };
  }

  if (said === "PHOTO") {
    await whatsapp.saveSession(contact.id, STEPS.PHOTO, draft);
    return { step: STEPS.PHOTO, reply: "Send the photograph of the result sheet now." };
  }

  /**
   * A photograph sent out of the blue.
   *
   * ── THE FASTEST PATH THROUGH THE WHOLE PRODUCT ─────────────────────────
   * An agent who simply photographs the sheet has done the entire return in
   * one action. The reader proposes every figure, the agent says yes, and
   * eleven numbers typed in the dark become one confirmation. When the
   * reading does not hold together the bot falls back to asking, so this is
   * a shortcut that can never become a wrong answer.
   */
  if (kind === "image" && mediaId && (step === STEPS.IDLE || step === STEPS.UNIT)) {
    const read = await readSheetIfPossible({ contact, draft, mediaId, imageBytes });

    if (read?.parsed?.usable) {
      const parsed = read.parsed;
      const unitCode = parsed.unitCode ?? draft.unitCode ?? contact.unitCode;

      if (unitCode) {
        const filled = {
          ...draft,
          unitCode,
          registered: parsed.registered ?? undefined,
          accredited: parsed.accredited,
          rejected: parsed.rejected ?? 0,
          votes: [...parsed.votes, Math.max(0, parsed.accredited - (parsed.rejected ?? 0) - parsed.sum)],
          repName: parsed.repName ?? null,
          mediaId,
          readId: read.id,
          fromSheet: true,
          /* ── THE READING IS KEPT, NOT JUST USED ────────────────────────
             Two things depend on it surviving this step.

             The return needs a match record. These figures came off the
             sheet, so they agree with it by construction — but "agreed with
             its photograph" and "had no photograph" are different facts about
             a return, and without this the strongest-corroborated filing the
             product can produce would be stored as the weakest.

             And the agent may answer NO. That branch re-asks for the figures,
             and if the reading did not survive to meet them, an agent could
             send a sheet, decline what it said, and then type anything at all
             with nothing to check it against. The hard block would have had a
             door in it exactly where somebody looking for one would push. */
          sheetParsed: parsed,
          sheetMatch: matchRecord(matchSheet(parsed, {
            registered: parsed.registered,
            accredited: parsed.accredited,
            rejected: parsed.rejected ?? 0,
            votes: parsed.votes,
          })),
        };
        await whatsapp.saveSession(contact.id, STEPS.CONFIRM, filled);
        return {
          step: STEPS.CONFIRM,
          read,
          reply: `I read your result sheet.\n\n${summarise(filled)}${parsed.repName ? `\nPresiding officer: ${parsed.repName}` : ""}\n\nIs that right? Send YES to file it, NO to type the figures yourself, or CANCEL to stop.`,
        };
      }
    }

    /* Either there is no reader configured, or it could not make the figures
       add up. Either way the agent is asked, and told why. */
    await whatsapp.saveSession(contact.id, STEPS.UNIT, { ...draft, mediaId, readId: read?.id ?? null });
    return {
      step: STEPS.UNIT,
      read,
      reply: `Thank you for the sheet.${read?.note ?? ""}\n\nI will take the figures from you instead. What is your polling unit code?`,
    };
  }

  switch (step) {
    case STEPS.UNIT: {
      const at = parseUnitCode(text);
      if (!at) {
        return {
          step,
          reply: "That does not look like a polling unit code. It is four parts, like 08-03-07-012, printed at the top of the sheet.",
        };
      }
      await whatsapp.saveSession(contact.id, STEPS.ACCREDITED, { ...draft, unitCode: at.code });
      return {
        step: STEPS.ACCREDITED,
        reply: `Polling unit ${at.code}${at.stateName ? `, ${at.stateName}` : ""}.\n\nHow many voters were accredited? Send the number only.`,
      };
    }

    case STEPS.ACCREDITED: {
      const value = digits(text);
      if (value === null) return { step, reply: "Send the number of accredited voters, digits only. For example 412." };
      if (value > 3000) {
        return { step, reply: "That is higher than any polling unit register. Check the figure and send it again." };
      }
      await whatsapp.saveSession(contact.id, STEPS.REJECTED, { ...draft, accredited: value });
      return { step: STEPS.REJECTED, reply: `${value} accredited.\n\nHow many ballots were rejected? Send 0 if none.` };
    }

    case STEPS.REJECTED: {
      const value = digits(text);
      if (value === null) return { step, reply: "Send the number of rejected ballots, digits only. Send 0 if there were none." };
      if (value > draft.accredited) {
        return { step, reply: `You cannot reject more ballots than the ${draft.accredited} voters accredited. Check and send it again.` };
      }
      await whatsapp.saveSession(contact.id, STEPS.VOTES, { ...draft, rejected: value, votes: [] });
      return { step: STEPS.VOTES, reply: `${value} rejected.\n\nNow the votes. How many for ${parties[0].id}?` };
    }

    case STEPS.VOTES: {
      const value = digits(text);
      const votes = draft.votes ?? [];
      const index = votes.length;

      if (value === null) return { step, reply: `Send the number of votes for ${parties[index].id}, digits only.` };

      const next = [...votes, value];
      const valid = draft.accredited - draft.rejected;
      const running = next.reduce((sum, item) => sum + item, 0);

      if (running > valid) {
        /* Caught here rather than at the end, so the agent fixes the figure
           they are looking at instead of being told at the end that one of
           five numbers was wrong. */
        return {
          step,
          reply: `That brings the total to ${running}, but only ${valid} ballots can be counted at this unit, ${draft.accredited} accredited less ${draft.rejected} rejected. Check ${parties[index].id} and send it again.`,
        };
      }

      if (next.length < parties.length) {
        await whatsapp.saveSession(contact.id, STEPS.VOTES, { ...draft, votes: next });
        return { step: STEPS.VOTES, reply: `How many for ${parties[next.length].id}?` };
      }

      /* Whatever is left over is everybody else, which is how the rest of the
         product carries it, so the two can never disagree. */
      const withOthers = [...next, Math.max(0, valid - running)];
      const filled = { ...draft, votes: withOthers };

      /* ── THE PICTURE IS ALREADY HERE ON THE SECOND PASS ──────────────────
         An agent sent back to retype their figures has already photographed
         the sheet, and the reading of it is on the draft. Asking for the
         photograph again would be asking somebody in a dark schoolyard to
         repeat work we have already done, and would make the block feel like
         a fault in the app rather than a check. So the figures go straight
         back to the reading we hold. */
      if (filled.sheetParsed) return await reconcile(contact, filled);

      await whatsapp.saveSession(contact.id, STEPS.PHOTO, filled);
      return {
        step: STEPS.PHOTO,
        reply: `${summarise(filled)}\n\nNow send a photograph of the result sheet. If you cannot, send SKIP.`,
      };
    }

    case STEPS.PHOTO: {
      if (kind === "image" && mediaId) {
        const read = await readSheetIfPossible({ contact, draft, mediaId, imageBytes });
        return await reconcile(contact, {
          ...draft,
          mediaId,
          readId: read?.id ?? null,
          sheetParsed: read?.parsed ?? null,
          /* Why no comparison was possible, if it was not, so the reply can
             say so in the agent's own terms rather than going quiet. */
          sheetNote: read?.note ?? "",
        });
      }
      if (said === "SKIP") {
        /* ── SKIPPING IS ALLOWED, AND IT IS NOT THE SAME AS PASSING ──────────
           A booth with no working camera must still be able to report; losing
           the return would cost more than the corroboration is worth. But the
           return is filed with no sheet match at all rather than with one that
           looks like agreement, so the desk can tell the two apart at a
           glance and on the record. */
        await whatsapp.saveSession(contact.id, STEPS.CONFIRM, { ...draft, sheetMatch: null });
        return { step: STEPS.CONFIRM, reply: `Filing without a sheet.\n\n${summarise(draft)}\n\nIs that correct? Send YES to file it, or CANCEL to start again.` };
      }
      return { step, reply: "Send the photograph of the result sheet, or send SKIP if you cannot." };
    }

    case STEPS.CONFIRM: {
      if (said === "NO" || said === "N") {
        /* Rejecting a reading is not cancelling. The polling unit is still
           right, so only the figures are asked again.

           The reading of the sheet is kept too. Dropping it here would have
           let an agent walk out of a mismatch simply by answering NO: the
           retyped figures would meet no photograph on the way back through,
           and the check would be one keystroke wide. */
        await whatsapp.saveSession(contact.id, STEPS.ACCREDITED, {
          unitCode: draft.unitCode,
          mediaId: draft.mediaId,
          readId: draft.readId,
          sheetParsed: draft.sheetParsed ?? null,
          sheetAttempts: draft.sheetAttempts ?? 0,
        });
        return {
          step: STEPS.ACCREDITED,
          reply: "No problem, we will do it by hand. How many voters were accredited?",
        };
      }
      if (said !== "YES" && said !== "Y" && said !== "CONFIRM") {
        return { step, reply: "Send YES to file these figures, NO to type them yourself, or CANCEL to start again." };
      }
      return await commit(contact, draft);
    }

    default:
      return { step: STEPS.IDLE, reply: HELP };
  }
}


/* ── the picture against the figures ──────────────────────────────────────── */

/**
 * Hold the typed figures against the photographed sheet, and refuse to move on
 * if they disagree.
 *
 * ── THE HOLE THIS CLOSES ───────────────────────────────────────────────────
 * Until now these two halves never met. The reader checked the photograph
 * against itself — do these figures add up — and the conversation filed
 * whatever the agent had typed, having replied "Sheet received" without ever
 * asking whether they were the same figures. An agent could photograph a sheet
 * showing 90 for a party and type 190, and every check in the system passed:
 * the typed figures were arithmetically sound, the sheet was arithmetically
 * sound, and nothing anywhere compared them.
 *
 * ── AND WHY THE BLOCK IS HARD ──────────────────────────────────────────────
 * There is no override. An agent whose figures contradict their own
 * photograph retypes them until they do not, and no number of attempts unlocks
 * a filing the picture disagrees with. That is only defensible because of what
 * counts as a disagreement, and lib/sheet-match.js is strict about it: the
 * reading has to be self-consistent before it may contradict anybody, and a
 * figure the reader could not make out is compared to nothing rather than to
 * zero. A reader that cannot read the sheet does not block the return — it
 * files with no corroboration recorded, which is a different thing and is
 * stored as a different thing.
 *
 * ── WHY IT ASKS FOR EVERY FIGURE AGAIN ─────────────────────────────────────
 * The message names precisely which figures disagreed and what the sheet says,
 * so nobody is hunting. But the conversation restarts at the first figure
 * rather than at the offending one, which is the same path the existing NO
 * branch takes, for the same reason: a step machine that can re-enter itself
 * at any of eleven points is a step machine with eleven ways to end up holding
 * half of one return and half of another.
 */
async function reconcile(contact, draft) {
  const match = matchSheet(draft.sheetParsed ?? null, {
    registered: draft.registered,
    accredited: draft.accredited,
    rejected: draft.rejected,
    votes: draft.votes,
  });

  if (match.comparable && !match.agrees) {
    const attempts = (draft.sheetAttempts ?? 0) + 1;

    /* The unit and the reading survive; the figures do not. Keeping the
       reading is what lets the retyped figures be checked against the same
       photograph rather than against a fresh one the agent has to take. */
    await whatsapp.saveSession(
      contact.id,
      STEPS.ACCREDITED,
      {
        unitCode: draft.unitCode,
        mediaId: draft.mediaId,
        readId: draft.readId,
        sheetParsed: draft.sheetParsed,
        sheetAttempts: attempts,
      },
      attempts
    );

    return {
      step: STEPS.ACCREDITED,
      blocked: true,
      attempts,
      match,
      reply: `${mismatchMessage(match, { channel: "whatsapp" })}\n\nHow many voters were accredited?`,
    };
  }

  await whatsapp.saveSession(contact.id, STEPS.CONFIRM, {
    ...draft,
    sheetMatch: matchRecord(match),
  });

  return {
    step: STEPS.CONFIRM,
    match,
    reply: match.comparable
      ? `Sheet received, and it agrees with your figures.\n\n${summarise(draft)}\n\nIs that correct? Send YES to file it, or CANCEL to start again.`
      : `Sheet received.\n\n${summarise(draft)}${draft.sheetNote ?? ""}\n\nIs that correct? Send YES to file it, or CANCEL to start again.`,
  };
}

/* ── the sheet reader ─────────────────────────────────────────────────────── */

/**
 * Read a photographed sheet, if there is anything to read it with.
 *
 * Always returns something rather than throwing. A reader that is not
 * configured, an image that will not download, a page of text that makes no
 * sense: all three end the same way, with the bot asking its questions, and
 * none of them may take the conversation down with them.
 */
async function readSheetIfPossible({ contact, draft, mediaId, imageBytes }) {
  if (!visionAvailable()) {
    return { note: "", parsed: null, id: null, reason: "no reader configured" };
  }

  const bytes = imageBytes ?? (await downloadMedia(mediaId));
  if (!bytes) {
    return { note: "\n\nI could not open the picture.", parsed: null, id: null, reason: "download failed" };
  }

  const read = await readSheet(bytes);
  if (!read.ok) {
    return { note: "\n\nI could not read the picture.", parsed: null, id: null, reason: read.reason };
  }

  const parsed = read.parsed;

  /* ── WHICH PROJECT THIS READING BELONGS TO ──────────────────────────────
     A message has no cookie and no reader, so the same rule the filing uses
     applies here: whatever project is actually running. Where none is, the
     reading is still returned — the conversation carries on and the agent is
     still asked to confirm the figures — but it is not stored, because a row
     with no project is a row nobody will ever find. */
  const project = await elections.active();

  const id = project ? await sheetReads.record({
    electionId: project.id,
    contactId: contact.id,
    unitCode: parsed.unitCode ?? draft.unitCode ?? contact.unitCode ?? null,
    mediaId,
    /* Null when a model did the reading: it returns figures rather than a
       page of text, and `parsed` below is the whole of what it said. */
    rawText: read.text,
    parsed,
    confidence: read.confidence,
    reader: read.reader ?? null,
  }) : null;

  return {
    id,
    parsed,
    confidence: read.confidence,
    /* The agent is told what went wrong in their own terms. "Could not
       reconcile the arithmetic" is not something to send to somebody standing
       in a schoolyard. */
    note: parsed.usable
      ? ""
      : `\n\nI could read some of the sheet but not all of it: ${parsed.problems[0] ?? "the figures did not add up"}.`,
  };
}

/**
 * Fetch the picture from Meta.
 *
 * Two calls, not one: the first exchanges the media id for a short-lived URL,
 * the second downloads it, and both need the access token. Meta expires that
 * URL in minutes, which is why the bytes are read now rather than a link
 * being stored for later.
 */
async function downloadMedia(mediaId) {
  const token = process.env.WHATSAPP_TOKEN;
  if (!token || !mediaId) return null;

  try {
    const lookup = await fetch(`https://graph.facebook.com/v21.0/${mediaId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!lookup.ok) return null;

    const { url } = await lookup.json();
    if (!url) return null;

    const file = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    if (!file.ok) return null;

    return Buffer.from(await file.arrayBuffer());
  } catch {
    return null;
  }
}

/* ── where the coordinator is ─────────────────────────────────────────────── */

function describePlace(location) {
  const at = `${Number(location.latitude).toFixed(5)}, ${Number(location.longitude).toFixed(5)}`;
  return location.name ? `Sent a location: ${location.name} (${at})` : `Sent a location: ${at}`;
}

/**
 * Bank a position and say something useful back.
 *
 * ── WHY THE DISTANCE IS QUOTED, AND WHAT IT IS NOT ─────────────────────────
 * If we know where the unit is, the agent is told how far from it they appear
 * to be. That is a courtesy and a check in one: a coordinator who has walked
 * to the wrong school finds out immediately, from a number rather than from
 * an accusation. It corroborates a filing and never authorises one, because a
 * phone fix drifts and a booth genuinely does get moved across a compound.
 */
async function recordPlace(contact, location) {
  const unitCode = contact.unitCode ?? null;
  const known = unitCode ? await units.at(unitCode) : null;

  const distance =
    known?.lat != null && known?.lon != null
      ? metresBetween(location.latitude, location.longitude, known.lat, known.lon)
      : null;

  await positions.record({
    contactId: contact.id,
    unitCode,
    lat: location.latitude,
    lon: location.longitude,
    accuracy: location.accuracy ?? null,
    label: location.name ?? location.address ?? null,
    distance,
  });

  /* The first fix from a unit we have never placed becomes that unit's
     position. Somebody standing at a booth is the best source we will ever
     have for where that booth is. */
  if (unitCode && known && known.lat == null) {
    /* The registry is keyed by (election, code), so a project has to be named.
       No project running means there is nowhere for this booth to be recorded,
       and the fix is still stored against the contact above — the position is
       not lost, it simply has no unit row to attach itself to yet. */
    const running = await elections.active();
    if (running) {
      await units.register({
        electionId: running.id,
        code: unitCode,
        lat: location.latitude,
        lon: location.longitude,
      });
    }
  }

  if (distance !== null && distance > 2000) {
    return {
      located: true,
      reply: `Location received. It puts you about ${Math.round(distance / 100) / 10} kilometres from where we have your polling unit. If that is wrong, tell the desk, and carry on either way.`,
    };
  }

  return {
    located: true,
    reply: unitCode
      ? `Location received. The desk can see you at unit ${unitCode}.`
      : "Location received. The desk can see you.",
  };
}

/** Great circle distance in metres. */
function metresBetween(lat1, lon1, lat2, lon2) {
  const R = 6_371_000;
  const toRad = (value) => (value * Math.PI) / 180;
  const dLat = toRad(lat2 - lat1);
  const dLon = toRad(lon2 - lon1);
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2;
  return Math.round(2 * R * Math.asin(Math.sqrt(a)));
}

/** The figures read back, so somebody confirms what they actually said. */
function summarise(draft) {
  const votes = draft.votes ?? [];
  const lines = parties.map((party, index) => `${party.id}: ${votes[index] ?? 0}`);
  if (votes.length > parties.length) lines.push(`Others: ${votes[parties.length]}`);
  return [
    `Unit ${draft.unitCode}`,
    `Accredited: ${draft.accredited}`,
    `Rejected: ${draft.rejected}`,
    ...lines,
  ].join("\n");
}

/**
 * Write the return, but only for a number an operator has tied to an account.
 *
 * A stranger's figures are kept as a draft on the desk instead. That is the
 * whole trust boundary of this channel: WhatsApp proves somebody controls a
 * phone, and nothing more, so it can never on its own put a number into the
 * count.
 */
async function commit(contact, draft) {
  if (contact.status !== "VERIFIED" || !contact.userId) {
    await whatsapp.saveSession(contact.id, STEPS.DONE, { ...draft, held: true });
    return {
      step: STEPS.DONE,
      held: true,
      reply: "Thank you. Your figures are with the desk and are waiting for an officer to confirm who you are before they are counted. You will get a message when that is done.",
    };
  }

  /* ── THE SHAPE OF A VOTE RECORD IS NOT OURS TO CHOOSE ───────────────────
     Returns are stored keyed by party, {"APC": 34, "PDP": 30, ...}, and 485
     of them already are. Filing an array here produced rows the rest of the
     product could not read: the same column held two shapes, and everything
     downstream had to guess which. The conversation collects figures in party
     order because that is the order it asks in; it converts before it writes. */
  const ordered = draft.votes ?? [];
  const votes = {};
  for (const [index, party] of parties.entries()) votes[party.id] = ordered[index] ?? 0;
  const spare = ordered[parties.length] ?? 0;
  if (spare > 0) votes.OTH = spare;

  const registered = draft.registered ?? draft.accredited;
  const at = parseUnitCode(draft.unitCode);

  const screening = screenReturn({
    unitCode: draft.unitCode,
    registered,
    accredited: draft.accredited,
    rejected: draft.rejected,
    votes: ordered,
  });

  /* The newest position from this coordinator rides along with the return, so
     the filing carries a fix even though it arrived over a channel that has
     no idea what a GPS reading is. */
  const fix = await positions.latestFor(contact.id);

  /* A message has no reader and no cookie, so it files into whatever election
     is actually running. Refusing beats guessing: filing a governorship return
     into a closed presidential project is not a small mistake. */
  const project = await elections.active();
  if (!project) {
    return "No election is running at the moment, so this return has nowhere to go. Your situation room needs to start a project first.";
  }

  /* ── WHICH BALLOT PAPER THIS IS ─────────────────────────────────────────
     The conversation never asks, because a step that asks costs a round trip
     to a phone on a rural signal for an answer that is the same all evening.
     It files into the project's headline contest — the one the day is named
     after — which is the only position a project is guaranteed to hold. An
     agent who has counted five ballots files the other four on the screen,
     where picking a position costs a tap and nothing else. */
  /* The unit registers itself as its return arrives, which is what makes the
     ward, local government and state totals fill in behind it without anybody
     maintaining a list. Done here rather than earlier because the registry is
     keyed by (election, code) and the project is not known until above. */
  await units.register({
    electionId: project.id,
    code: draft.unitCode,
    registered,
    repName: draft.repName ?? null,
    source: "WHATSAPP",
  });

  await results.file({
    electionId: project.id,
    race: defaultRace(project),
    unitCode: draft.unitCode,
    stateCode: contact.stateCode ?? at?.stateCode ?? draft.unitCode.slice(0, 2),
    registered,
    accredited: draft.accredited,
    rejected: draft.rejected,
    votes,
    note: draft.fromSheet ? "Filed over WhatsApp, read from the sheet" : "Filed over WhatsApp",
    submittedBy: contact.userId,
    source: "WHATSAPP",
    repName: draft.repName ?? null,
    /* Null where no sheet was compared, which the desk must be able to tell
       apart from a sheet that was compared and agreed. A return that reaches
       this line having disagreed with its photograph does not exist: reconcile
       sends those back rather than forward. */
    sheetMatch: draft.sheetMatch ?? null,
    position: fix ? { lat: fix.lat, lon: fix.lon, accuracy: fix.accuracy, distance: fix.distance } : null,
  });

  /* What the reader proposed is marked accepted against what was filed, so the
     difference between machine and human stays on the record. */
  if (draft.readId) {
    await sheetReads.accept(draft.readId, {
      accredited: draft.accredited,
      rejected: draft.rejected,
      votes: ordered,
    });
  }

  await whatsapp.endSession(contact.id);

  /* ── AND ON TO THE HUB ──────────────────────────────────────────────────
     The webhook already forwards the raw message the moment it lands. This is
     a different thing and both are needed: the message is what arrived, and
     these are the figures a six-turn conversation resolved it into. A hub
     holding only the messages would hold the evidence and not the count.

     ── THE CHANNEL TRAVELS WITH IT ──────────────────────────────────────
     Marked WHATSAPP rather than left to be inferred. Data Bank caps WhatsApp
     at a trust ceiling of 62 — Meta strips EXIF, and a location arrives as a
     bare pin with no accuracy radius and no device attestation — and a figure
     that arrived this way must never be weighed downstream as though it came
     from an attested device. See SOURCES in that project's lib/taxonomy.js.

     Never awaited, like every other call site: an agent on a failing rural
     signal must not wait on a hub to finish filing. See lib/databank.js. */
  forwardToDataBank({
    kind: KIND.RESULT_FIGURES,
    externalId: `poll360:result:${project.id}:${defaultRace(project)}:${draft.unitCode}`,
    sender: contact.phone ?? null,
    /* ── NO HASH ON THIS PATH, AND THAT IS STATED ─────────────────────
       A sheet sent over WhatsApp is read and matched, but the draft the
       conversation carries between turns holds the *reading* — parsed
       figures, a match record, a reader id — and never the bytes, so there
       is nothing here to hash. An empty list is the honest answer; inventing
       a field that is always undefined is how the pairing came to be broken
       everywhere else in this product. See app/field/actions.js. */
    mediaHashes: [],
    payload: {
      unitCode: draft.unitCode,
      stateCode: contact.stateCode ?? at?.stateCode ?? draft.unitCode.slice(0, 2),
      race: defaultRace(project),
      electionId: project.id,
      registered,
      accredited: draft.accredited,
      rejected: draft.rejected,
      votes,
      channel: "WHATSAPP",
      /* Whether the agent typed these or a reader pulled them off a
         photograph they sent. The bot knows; nothing downstream can tell
         without being told. */
      readBy: draft.fromSheet ? "MACHINE" : "HUMAN",
      position: fix
        ? { lat: fix.lat, lon: fix.lon, accuracy: fix.accuracy, distance: fix.distance }
        : null,
      flags: screening?.flags ?? [],
      filedAt: new Date().toISOString(),
    },
  });

  const flagged = screening?.flags?.length ?? 0;
  return {
    step: STEPS.DONE,
    filed: true,
    flags: screening?.flags ?? [],
    reply: flagged
      ? `Filed for unit ${draft.unitCode}. The desk has been asked to look at it, so somebody may call you to check a figure. That is routine.`
      : `Filed for unit ${draft.unitCode}. Thank you.`,
  };
}

async function statusFor(contact) {
  if (!contact.unitCode) {
    return "Your number is not yet tied to a polling unit. Send RESULT and the desk will match you up.";
  }
  const running = await elections.active();
  if (!running) return "No election is running at the moment.";
  const filed = await results.forUnit(contact.unitCode, running.id, defaultRace(running));
  if (!filed) return `Nothing filed yet for unit ${contact.unitCode}. Send RESULT to file now.`;
  return `Unit ${contact.unitCode} was filed and is marked ${String(filed.status).toLowerCase()}.`;
}

export { HELP };
