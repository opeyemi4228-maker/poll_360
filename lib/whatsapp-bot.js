...
/* An agent identifying themselves by the code they were given at sign-up.
   The same answer the web form gets — see lib/agent-login.js. */
import { looksLikeAgentCode } from "./agent-code.js";
import { accountForCode } from "./agent-login.js";
...
/* The hub every input is also delivered to. Fire-and-forget by construction
   — see lib/dumpsite.js. */
import { KIND, forwardToDumpSite } from "./dumpsite.js";
...
async function advance({ contact, session, step, text, kind, mediaId, imageBytes }) {
  ...
  if (looksLikeAgentCode(text)) {
    const person = await accountForCode(text);
...
async function commit(contact, draft) {
  ...
  /* ── AND ON TO THE HUB ──────────────────────────────────────────────────
     ...
     Marked WHATSAPP rather than left to be inferred. DumpSite caps WhatsApp
     ...
     signal must not wait on a hub to finish filing. See lib/dumpsite.js. */
  forwardToDumpSite({
