...
import { issueAgentCode } from "./agent-code.js";
...
const shape = (row) =>
  row ? {
    ...
    createdAt: row.created_at ? at(row.created_at) : null,
    /* ── THE CODE ITSELF IS NEVER ON THIS OBJECT ──────────────────────────
       Only whether one exists, when it was cut, and its last two symbols —
       enough for a desk on the telephone to say "the one ending QX" and be
       understood. The hash follows the same rule as the password hash above
       it: asked for explicitly by the one function that verifies a sign-in,
       and never riding along on the object a screen renders. */
    hasCode: Boolean(row.code_hash),
    codeHint: row.code_hint ?? null,
    codeIssuedAt: row.code_issued_at ? at(row.code_issued_at) : null,
...
export const coordinators = {
  ...
  /**
   * Give one agent a code, and return it once.
   *
   * ── THE PLAIN CODE IS RETURNED AND NEVER STORED ──────────────────────────
   * This is the only moment it exists in this system. It goes to the screen
   * that shows it to the agent and nowhere else — not to the audit trail, not
   * to a log line, not into the row. An administrator asked for a forgotten
   * code issues a new one, which is the right answer anyway: a code somebody
   * has lost may have been lost to a person rather than to memory.
   *
   * Re-issuing replaces the old one. There is exactly one live code per
   * account, so a code read out to the wrong person on Tuesday stops working
   * the moment a new one is cut.
   */
  async issueCode(id, { hash }) {
    const person = await coordinators.byId(id);
    if (!person?.unitCode) return null;

    const code = issueAgentCode(person.unitCode);
    const stored = await hash(code);

    (await db.prepare(
      `UPDATE coordinators
          SET code_hash = ?, code_hint = ?, code_issued_at = now()
        WHERE id = ?`
    ).run(stored, code.slice(-2), id));

    return code;
  },

  /**
   * Every account that could hold this code, by the booth inside it.
   *
   * ── WHY THIS NARROWS BEFORE IT VERIFIES ──────────────────────────────────
   * A code cannot be looked up directly: it is hashed with a per-row salt, so
   * there is no index to search. Verifying against every account in the
   * country would be forty thousand slow hashes per sign-in attempt, which is
   * a denial of service anybody can trigger.
   *
   * The booth in the code narrows it to the handful of people at one polling
   * unit. That is not a security decision and must not be read as one — the
   * booth half is public and anybody can write it — it is only how the search
   * is made finite. The secret half still has to match a stored hash.
   */
  async holdersAtUnit(unitCode) {
    const rows = await db
      .prepare(
        `SELECT id, code_hash FROM coordinators
          WHERE unit_code = ? AND code_hash IS NOT NULL AND disabled_at IS NULL`
      )
      .all(unitCode);
    return rows.map((row) => ({ id: row.id, codeHash: row.code_hash }));
  },

  /** Note that a photograph was stored for this agent. */
  async attachPhoto(id, mediaId) {
    (await db.prepare("UPDATE coordinators SET photo_id = ? WHERE id = ?").run(mediaId, id));
  },
  ...
  /**
   * The row an agent confirmed by Data Bank signs in and files as.
   *
   * ── DATA BANK HOLDS THE LIST; THIS HOLDS THE AUTHOR ────────────────────
   * Who the agents are lives in Data Bank. What this table still does is give
   * a signed-in agent a session and give every result they file an author, so
   * a confirmed agent is kept here as one row, found by their Data Bank id.
   * Their name, number and polling unit are refreshed from the list on every
   * sign-in, because the list is where a correction is made.
   *
   * An agent who held an account here before the list moved to Data Bank is
   * matched by phone number the first time, so their earlier results keep the
   * same author rather than splitting across two rows.
   */
  async mirrorAgent(agent) {
    const at = agent?.pollingUnitCode ? parseUnitCode(agent.pollingUnitCode) : null;
    if (!agent?.id || !at) return null;

    /* This table stores numbers as 234 and ten digits; the list stores 0 and ten. */
    const digits = String(agent.phone ?? "").replace(/\D/g, "");
    const phone = digits.length === 11 && digits.startsWith("0") ? `234${digits.slice(1)}` : digits || null;

    const linked = await db.prepare("SELECT id FROM coordinators WHERE databank_agent_id = ?").get(agent.id);
    const earlier = linked
      ? null
      : phone
        ? await db
            .prepare("SELECT id FROM coordinators WHERE phone = ? AND databank_agent_id IS NULL")
            .get(phone)
        : null;
    const id = linked?.id ?? earlier?.id ?? null;

    if (id) {
      (await db.prepare(
        `UPDATE coordinators
            SET name = ?, phone = ?, unit_code = ?, state_code = ?, status = 'ACTIVE',
                disabled_at = NULL, databank_agent_id = ?
          WHERE id = ?`
      ).run(agent.fullName, phone, at.code, at.stateNumber, agent.id, id));
      return coordinators.byId(id);
    }

    const fresh = randomUUID();
    /* No password: agents sign in with a code only. The column is NOT NULL
       from before, so it holds a marker no password check could ever match. */
    (await db.prepare(
      `INSERT INTO coordinators
         (id, name, phone, password_hash, unit_code, state_code, status, approved_at, databank_agent_id)
       VALUES (?, ?, ?, 'code-only', ?, ?, 'ACTIVE', now(), ?)`
    ).run(fresh, agent.fullName, phone, at.code, at.stateNumber, agent.id));

    return coordinators.byId(fresh);
  },
};
