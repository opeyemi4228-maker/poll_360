...
/* ══════════════════════════════════════════════════════════════════════════
   THE BAND ACROSS THE TOP OF THE WATCH CONSOLE

   ── WHY THIS IS HERE AND NOT IN THE COMPONENT ─────────────────────────────
   It was in the component, and on the evening it was written it took the
   alerts console down twice: once reading `raiseAlerts`'s return value as an
   array when it is an object, and once reading a variable that had been
   renamed underneath it. Neither is exotic. Both are the ordinary cost of
   doing arithmetic inside a render function.

   What made them expensive is that nothing in this repository could catch
   them. The build cannot: an undefined name inside a component is a valid
   module until the line runs. The tests cannot: node's runner has no DOM and
   no path aliases, so no test here renders a client component, and none ever
   will without a toolchain this project has deliberately not taken on.

   So the rule this file now follows is the one lib/pulse.js and
   lib/executive.js already followed: the arithmetic lives in a plain module
   that a test can call, and the component only draws what it is handed. The
   shape mismatch that caused the outage is a failing test now — see
   tests/alerts.test.js — rather than a white screen.
   ══════════════════════════════════════════════════════════════════════════ */

/**
 * Severity, worst first. The order every count below is indexed by, so a
 * caller can hand rank 0 the loudest colour without knowing the names.
 */
export const SEVERITY_ORDER = ["CRITICAL", "SERIOUS", "INFO"];

/**
 * Everything the watch console's picture band draws.
 *
 * @param alerts     what `raiseAlerts` returned — an OBJECT, `{ level, alerts,
 *                   … }`, not a list. Null is a room with nothing raised.
 * @param incidents  the field reports, which are a plain array.
 */
export function watchBand({ alerts = null, incidents = [] } = {}) {
  const rows = alerts?.alerts ?? [];

  /* Normal is a row in that list and is not a warning. Counting it as one
     would put a room with nothing wrong at "1 thing to look at", which is the
     fastest way to teach people to ignore the number. */
  const raisedRows = rows.filter((row) => row.level !== "NORMAL");

  /* Taken from the report, never recomputed: this file says plainly at the
     top that the room's one word must not be worked out a second way. */
  const level = alerts?.level ?? "NORMAL";

  const bySeverity = SEVERITY_ORDER.map((id) => ({
    id,
    count: incidents.filter((row) => row.severity === id).length,
  }));

  /* Per state: the worst thing reported there, and how many. Rank is an index
     into SEVERITY_ORDER, so 0 is the loudest — a caller colours by it without
     repeating the severity names. A state with nothing reported is simply
     absent, which is what lets a map leave it blank rather than drawing it as
     a low number. */
  const byState = {};
  for (const item of incidents) {
    if (!item.stateCode) continue;
    const rank = Math.max(0, SEVERITY_ORDER.indexOf(item.severity));
    const held = byState[item.stateCode];
    byState[item.stateCode] = {
      rank: held ? Math.min(held.rank, rank) : rank,
  ...
  return {
    level,
    rank: LEVELS[level]?.rank ?? 0,
    label: LEVELS[level]?.label ?? "Normal",
    raisedRows,
    /* The one figure the arc is about: everything a person would have to look
       at, from both halves of this console. */
    raised: raisedRows.length + incidents.length,
    reports: incidents.length,
    /* The two severities that mean somebody has to do something now. */
    needSomebody: bySeverity[0].count + bySeverity[1].count,
    bySeverity,
    byState,
