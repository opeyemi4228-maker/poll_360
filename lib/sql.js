...
export const sql = globalForSql.poll360Sql ?? neon(connectionString());
if (process.env.NODE_ENV !== "production") globalForSql.poll360Sql = sql;
...
export async function transaction(build) {
  const statements = build(
    (text, ...args) => sql.query(positional(text), args)
  );
