...
export const watch = {
   ...
   */
  async coordinators(electionId, race, territory = null) {
    ...
    /* ── THE WATCH IS NARROWED TOO, AND BOTH HALVES OF IT ─────────────────
       A room that may not read a booth's return has no business being shown
       the agent standing at it: the map would draw a dot in a place none of
       its figures come from, and `summary` would count that booth into a
       coverage denominator the room cannot see the numerator of.

       Applied to each half of the union separately, because a UNION takes one
       WHERE per branch and a filter on only one of them is the exact shape of
       bug this query already carries a comment about — half the watch, quietly.
       The columns differ by branch: an account's booth is `u.scope`, a
       coordinator's is `c.unit_code`. */
    const staff = within(territory, "u.scope");
    const booth = within(territory, "c.unit_code");
      ...
      .prepare(
        `SELECT u.id, u.name, u.scope, u.last_login_at, 'staff' AS kind,
          ...
          WHERE u.role = 'PU_AGENT' AND u.scope IS NOT NULL
            AND u.status = 'ACTIVE' AND u.disabled_at IS NULL${staff.sql}
         ...
         SELECT c.id, c.name, c.unit_code AS scope, c.last_login_at, 'coordinator' AS kind,
          ...
          WHERE c.unit_code IS NOT NULL${booth.sql}
    ...
    return [...byUnit.values()].map((row) => {
      ...
      return {
            ...
            : row.distance_m == null
              ...
              : row.distance_m <= 250
                ...
                : row.distance_m <= 2000
                  ? "near"
                  : "far",
        at: row.submitted_at ? at(row.submitted_at) : null,
        ...
        fresh: row.submitted_at
          ? Date.now() - at(row.submitted_at).getTime() < 10 * 60 * 1000
          : false,
        lastSeen: row.last_login_at ? at(row.last_login_at) : null,
      };
    });
  },
