...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   All 37 of them, now that every state has been imported. It was not always:
   the list named all thirty-seven while nine had never been imported, and a
   static import of a file that is not on disk is not a graceful absence — it
   is a build failure. Keep the two in step.

   A state absent from here must also be absent from the index, so `membersAt`
   answers null for it. Null is unknown, which is true. Nought would be a claim
   about the party that nobody has made.
...
const FILES = {
  ADC: {
    ...
    OSU: () => import("./data/members-osu-adc.js"),
    ...
    TAR: () => import("./data/members-tar-adc.js"),
...
export function qualityOf(party, stateCode) {
  /* ── null STATE MEANS THE WHOLE REGISTER ───────────────────────────────
     The screen shows this panel beside whichever map is drawn, and beside
     the national map it was showing one state's findings under a heading
     that named that state — true, and not what the reader was looking at.
     Asked for the country, the defects are summed across every state so the
     caveat matches the figure it sits next to. */
  const register = stateCode ? registerFor(party, stateCode) : null;
  if (stateCode && !register) return null;

  const found = stateCode ? (register.quality ?? {}) : acrossStates(party);
  ...
  if (outside.length) {
    notes.push({
      ...
      severity: "INFO",
      says: `${outside.length} local government${outside.length === 1 ? "" : "s"} in ${stateCode ? `this register are not in ${register.state}` : "these registers are not in the state they were filed under"}: ${outside
        .slice(0, 6)
        .map(([name]) => name)
        .join(", ")}${outside.length > 6 ? ` and ${outside.length - 6} more` : ""}`,
  ...
  const members = stateCode
    ? register.members
    : Object.values(INDEX[party]?.states ?? {}).reduce((sum, row) => sum + row.members, 0);

  return {
    party,
    state: stateCode ? register.state : "national",
    members,
    notes,
    ...
    votingAge: members - (found.underEighteen ?? 0),
  };
}

/**
 * Every state's defects, added together.
 *
 * Counts sum. The two fields that are not counts — the local governments filed
 * outside their state, and the headings resolved to a different spelling — are
 * dictionaries, and they are merged with the state's code on each key so a
 * national note can still name the place a finding came from.
 */
function acrossStates(party) {
  const states = Object.entries(INDEX[party]?.states ?? {});
  if (!states.length) return null;

  const found = { outsideState: {}, lgaSpellingsResolved: {} };
    ...
    for (const [key, value] of Object.entries(state.quality ?? {})) {
      if (typeof value === "number") {
        ...
        for (const [name, held] of Object.entries(value)) {
          /* Keyed by state, because "Ohafia" is a finding about Ebonyi's
             register and about Abia's list, and two states can report the
             same name for different reasons. */
          found[key][`${name} (${code})`] = held;
        }
      }
    }
  }
