 ...
 *
 * ── AND WHY A GROUND CAN BREAK THE TIE ─────────────────────────────────────
 * `stateCode` is the state an account's territory sits in, where it has one.
 * It changes nothing about a stated preference — a cookie is a decision and
 * always wins — but it decides where an account lands the very first time it
 * signs in, before it has made one.
 *
 * Without it that first landing is "the project with the newest return
 * anywhere in the country", which for a campaign issued an Adamawa district
 * is a national replay narrowed to seven local governments: a correct screen,
 * showing the right ground of the wrong election, with nothing on it obviously
 * wrong. They would have to know a project switcher existed to find their own
 * night. A project scoped to the state they hold is the better guess, and it
 * is only ever a guess: one click changes it and the cookie remembers.
 */
export const currentElection = cache(async function currentElection(stateCode = null) {
  ...
  /* No preference stated, and this account holds ground somewhere. The
     newest active project fought in that state beats one fought elsewhere,
     however busy the other one is. A project with an empty scope is the whole
     federation and is not a match for anybody in particular. */
  if (stateCode) {
    const mine = (await listElections()).find(
      (row) => row.status === "ACTIVE" && row.scopeStates.includes(stateCode)
    );
    if (mine) return mine;
  }

  /* Open on the night that is actually happening, rather than on whichever
 ...
 * A project is the election day. The contests held on it are read one at a
 * time, because they are separate counts that must never be summed —
 ...
 * and meaningless across them: the contests of one election day are not the
 * contests of another, and carrying a choice over is how somebody
