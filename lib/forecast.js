 ...
 */
export function project({ swing = {}, turnout = 1, levers = null } = {}) {
  const rows = states2023.map((state) => {
...
export function winCondition(projection) {
  ...
  return parties.map((party, index) => {
    ...
    return {
      ...
      quarterStates,
      quarterFct,
      /* Two readings, both shown. */
      spreadStrict: quarterStates >= 24 && quarterFct,
      spreadPlain: quarterStates >= 24,
      shortBy: Math.max(0, 24 - quarterStates),
    };
 ...
 */
export function turnoutSensitivity(swing) {
  return [0.7, 0.85, 1, 1.15, 1.3].map((level) => {
    const projection = project({ swing, turnout: level });
