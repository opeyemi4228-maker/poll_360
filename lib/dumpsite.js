 ...
 *  actually standing there". DumpSite answers the question underneath both —
 ...
 * that is barely there, presses "file". If forwarding that return to DumpSite
 * is on the path between them and a saved result, then DumpSite being slow,
 ...
 * DumpSite is the hub they are *also* delivered to.
 *
 ...
 * replayed by `npm run dumpsite:replay`. A hub that quietly missed four
...
const ENDPOINT = process.env.DUMPSITE_URL ?? "";
const KEY = process.env.DUMPSITE_API_KEY ?? "";

/**
 * How long to wait before giving up on the hub.
 *
 * Short on purpose. This is not on a user's path, so a long timeout buys
 * nothing and costs a held connection per return on the busiest hour of the
 * year. Anything slower than this is a hub having a bad night, and the outbox
 * is the right answer to that rather than patience.
 */
const TIMEOUT_MS = 4000;

/** The kinds DumpSite sorts into — see lib/taxonomy.js in that project. */
export const KIND = {
 ...
 * @param payload     the body. Sealed and chained by DumpSite on receipt.
 * @param externalId  this product's own id for the thing, so a replay cannot
 *                    create a second copy — DumpSite dedupes on it.
 ...
 */
export async function sendToDumpSite({
  kind,
  payload,
  externalId = null,
  sender = null,
 ...
 */
export function forwardToDumpSite(input) {
  /* Errors inside `sendToDumpSite` are already caught and held; this catch is
  ...
  void sendToDumpSite(input).catch(() => {});
}
