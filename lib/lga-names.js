...
function bookFor(stateNumber) {
  ...
  try {
    ...
    /* A state we hold no unit list for is named by its numbers. */
    book = null;
  }
...
/**
 * The names of every place a unit code names: state, local government, ward
 * and the polling unit itself, from INEC's own unit list in public/geo/units.
 *
 * Used where a record leaves this product for somewhere that groups by place
 * and has no list of its own — Data Bank's Sheets board reads "Binji", not
 * "33/01". Each name is null where the list does not hold it; the code is
 * never guessed into a name.
 */
export function placeOf(unitCode) {
  const at = parseUnitCode(unitCode);
  if (!at) return null;

  const book = bookFor(at.stateNumber);
  const [, lgaNo, wardNo] = at.wardCode.split("/");
  const lga = book?.lgas?.find((row) => row.n === lgaNo) ?? null;
  const ward = lga?.wards?.find((row) => row.n === wardNo) ?? null;
  const unit = ward?.units?.find((row) => row.n === at.unitNo) ?? null;

  return {
    stateName: book?.name ?? at.stateName ?? null,
    lgaName: lga?.name ?? lgaNameFor(at.code),
    wardName: ward?.name ?? null,
    unitName: unit?.name?.replace(/\s+/g, " ").trim() || null,
  };
}
