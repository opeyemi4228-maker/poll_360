...
import { addVotes, foldIndex } from "./tally";
...
export function snapshot(board, cursor) {
  ...
  /* ── WHERE A POSITION GOES THAT THE BALLOT DOES NOT NAME ────────────────
     The accumulators below are sized from the ballot, and the loop that fills
     them used to run to `event.votes.length`. That was a fix for a real bug —
     it replaced a hardcoded `party < 5`, which silently dropped a sixth party
     — and it introduced the opposite one: an event carrying more positions
     than the board names wrote past the end of the array, and `undefined + n`
     is NaN. One such event turned the national total, every state total, every
     share and every bar width on the command dashboard into NaN. It showed up
     as "VOTES COUNTED NaN" over four full-width bars, because `width: NaN%` is
     dropped by the browser and a block element then fills its track.

     Neither dropping nor overflowing is right. A vote is a vote, so anything
     past the named positions folds into the "others" bucket, which exists for
     exactly this and is already counted in every total.

     A board with no such bucket has nowhere honest to put it: an unnamed
     figure added to the total would inflate every share computed against it,
     so it is left out of the count rather than allowed to distort it. That is
     a malformed board, and the surplus is reported so it cannot pass unseen. */
  const fold = foldIndex(slots);
  let unnamed = 0;
  ...
  for (let index = 0; index < limit; index += 1) {
    ...
    /* Was `party < 5`, which is the number of presidential parties and not a
       ...
       dropped it. */
    /* Into the state and into the nation, through the one function that
       knows what to do with a position the ballot does not name. */
    const added = addVotes(entry.votes, event.votes, fold);
    addVotes(national, event.votes, fold);
    entry.total += added.counted;
    unnamed += added.unnamed;
  }
  ...
  const byState = board.states.map((state, index) => {
    ...
    if (!entry.units) {
      /* A malformed board says so rather than quietly counting short. Not thrown:
     a room mid-count is not helped by a blank screen, and every figure around
     this one is still correct. */
  if (unnamed > 0 && typeof console !== "undefined") {
    console.warn(
      `snapshot: ${unnamed} votes arrived in positions this board does not name, and no "OTH" slot exists to hold them. They are not counted.`
    );
  }
  ...
  return { code: state.code, reported: false, units: 0, total: 0, coverage: 0 };
    ...
    return {
      ...
      coverage,
      /* ── THE `others` BUCKET NEVER COLOURS A STATE ────────────────────
         If it were ever ahead on partial returns the state reads as
         leading-unknown rather than being painted in a colour that names
         nobody.

         ── AND IT IS FOUND BY NAME, NOT BY POSITION ──────────────────────
         This read `leader.party < 4`, which is the presidential four with the
         bucket at index 4 — true of the 2023 replay this file was written
         for, and false of every live board. The live presidential ballot is
         nineteen slots (lib/races.js), and on it APC is 5, LP is 10, NDC 11,
         NNPP 12, PDP 13, SDP 15. Every one of those is `>= 4`, so a live
         count painted its states grey and reported "leading unknown" for all
         but the first four parties on the paper — a map that showed nothing
         on the night it mattered.

         The note above `slots` names this exact bug as the one it was fixing
         "one layer down"; the accumulator was widened and this line was not.
         The bucket is now identified the only way that survives a change of
         ballot: by its id. */
      leader: slots[leader.party]?.id === others.id ? null : leader.party,
