...
export function stripWake(text) {
  ...
  /* Not called by name at all: hand the sentence back untouched. Stripping a
     greeting from a sentence that never addressed the assistant would eat the
     first word of somebody's question. */
  if (!WAKE.test(raw)) return raw.trim();

  /* ── THE GREETING IN FRONT OF THE NAME GOES TOO ──────────────────────────
     Two of the three ways the name is matched do not include the greeting, so
     "Hi Poll360 AI" came back as "hi" rather than empty. That is not cosmetic:
     a non-empty remainder is treated as the first thing asked, so simply
     calling the assistant by name had it try to answer the word "hi" — and,
     finding nothing, go and look "hi" up on the web. Being greeted should
     produce a greeting. */
  const without = raw
    .replace(WAKE, " ")
    .replace(/^[\s,.]*(?:hi|hey|hello|ok|okay|yo|high)\b[\s,.]*/i, " ")
    .replace(/\s+/g, " ")
    .trim();
...
const SURFACES = [
  ...
  /* ── THE TWO SURFACES SOMEBODY ASKS FOR BY DESCRIPTION ────────────────
     Nobody says "take me to the pulse". They say "how are we doing", which is
     a question about a screen and not a question for the assistant to answer
     in prose — the screen answers it far better than a sentence can. */
  { tab: "pulse", words: ["overview", "the overview", "how are we doing", "the room", "command", "vital signs", "the pulse", "what's happening", "what is happening"] },
  { tab: "coverage", words: ["coverage", "the coverage", "the funnel", "funnel", "who has reported", "reporting rate", "completeness", "how much is in", "polling unit", "the polling unit", "unit intelligence", "polling unit intelligence", "booth", "the booth", "this booth"] },
  /* ── THE FOUR SURFACES ADDED WITH THE COMMAND SPLIT ───────────────────
     Each one carries the phrase somebody actually says out loud as well as
     its own name. "What needs attention" is the alerts screen and not a
     question for the assistant to answer in prose: the list answers it with
     numbers and doors, which is what the person asking is going to act on. */
  /* ── ALERTS AND THE FIELD REPORTS ARE ONE SCREEN ─────────────────────
     Both halves keep every word they had. Somebody who has always said
     "the incident feed" must not discover that the phrase stopped working
     because two tabs became one — the console it lands on is holding what
     they asked for, on the half it opens on. */
  { tab: "situations", words: ["situation", "situations", "the situations", "alert", "alerts", "the alerts", "escalations", "warnings", "what needs attention", "what requires attention", "what is on fire", "what's on fire", "report", "reports", "the reports", "incident", "incidents", "the incidents", "the feed", "situation feed", "report stream"] },
  { tab: "timeline", words: ["timeline", "the timeline", "the night", "what happened", "history of the night", "when did", "phases"] },
  { tab: "operations", words: ["operations", "the pipeline", "pipeline", "results operations", "the queue", "queue", "what is stuck", "what's stuck", "where is it stuck", "processing"] },
  /* The declared comparison lives on this surface now, so the words for it
     come here too — but not the bare word "declared", which belongs to the
     /gap room below and would be taken off it by the longest-match rule. */
  { tab: "integrity", words: ["integrity", "verification", "the checks", "anomalies", "the anomalies", "screening", "what looks wrong", "flags", "the flags", "against the declaration", "what was declared here", "declared figures here", "evidence", "the evidence", "data quality", "quality", "result sheet", "result sheets", "what did we capture", "capture"] },
  ...
  /* ══════════════════════════════════════════════════════════════════════
     THE TWO ANALYTICAL DASHBOARDS, AND EVERY WORD THAT USED TO REACH A TAB

     Seven surfaces became two. Every phrase anybody has been saying for
     months still works and lands on the dashboard now holding what they
     asked for — a phrase that quietly stops working is worse than one that
     never did, because nobody reports it, they just decide the assistant is
     unreliable.

     The split between the two is the split the screens make: anything that
     ends in a *finding* is Election Analytics, and anything that ends in a
     *commitment* — where to send people, what it costs, what we are aiming
     at — is Strategic Planning.
     ══════════════════════════════════════════════════════════════════════ */
  {
    tab: "analytics",
    words: [
      "analytics", "election analytics", "the analytics",
      /* The brief, asked for by its question rather than its name. Nobody
         says "open the strategic brief"; they say "where do we stand". */
      "brief", "the brief", "strategic brief", "strategy", "the strategy",
      "strategic intelligence", "executive", "overview",
      "where do we stand", "how are we doing overall", "our position", "the position",
      /* Geography, which was the classification map and party ground. */
      "geography", "geographic intelligence", "classification", "the classification",
      "classify", "party ground", "the party ground", "party strength",
      "where is the party strong", "where are we strong", "where are we weak",
      "stronghold", "strongholds", "battleground", "battlegrounds",
      "swing", "swing states", "weak areas",
      /* Candidates, which was parties and the ruling party. */
      "candidate", "candidates", "the parties", "party performance",
      "ruling party", "who governs",
      /* Historical and trends, which were Behaviour. */
      "historical", "the record", "history", "past elections", "compare elections",
      "trend", "trends", "trend detection", "what has been moving",
      /* Turnout analytics is a tab here; the turnout *map layer* keeps its
         own entry above and wins on the longest-match rule for "turnout
         layer". */
      "turnout analytics", "who stopped voting", "stayed home",
      /* Clusters, which is a tab of Geography now. */
      "cluster", "clusters", "density", "the cluster layer", "voters per unit",
    ],
  },
  {
    tab: "planning",
    words: [
      "planning", "strategic planning", "the plan", "deployment", "planning map",
      "coverage plan", "where should we focus", "where do we focus", "priorities",
      "the priorities", "priority states", "priority wards", "resource allocation",
      "resources", "grassroots", "grassroots strength", "grassroots intelligence",
      "opportunity", "opportunities", "target", "our target", "target tracking",
      /* Scenarios and the projection, which end in a decision about effort
         rather than in a finding about the country. */
      "scenario", "scenarios", "what if", "the projection", "projection",
      "the forecast", "forecast",
      "sample", "the sample", "sample design", "sample size", "quick count",
      "how many booths do we need", "margin of error",
    ],
  },
];
...
function headline(code) {
  ...
  /* The 2023 dataset's own column order, not the ballot's: `row.votes` is a
     positional array over that record and a literal list here would name the
     wrong party the day either one changes. */
  const ids = allParties.map((party) => party.id);
...
export function harvest(text, context = {}) {
  ...
  /* ── A NAME EARNS ITS FIGURES TOO ──────────────────────────────────────
     Naming a candidate used to put nothing on the board at all — the
     encyclopaedia card with their face arrived and the numbers did not,
     which on a planning board is the wrong half. The standings go up beside
     the face, so the two halves of "who is this and how did they do" are
     answered in one glance. */
  if (!specs.length && (findEveryone(q).length || findParty(raw))) {
    specs.push({ kind: "parties", place: null });
  }
...
/**
 * Correcting something it misheard.
 *
 * ── WHY A PRODUCT THAT LISTENS NEEDS THIS ──────────────────────────────────
 * Recognition gets names wrong, and the names in this room are exactly the
 * ones it gets wrong most. Without a way to correct it, the only repair
 * available is to say the whole sentence again, louder — which is both what
 * everybody does and the thing least likely to work, because the recogniser
 * will make the same mistake for the same reason.
 *
 * Saying "no, I meant Atiku" hands it the one word that went wrong, on its
 * own, with none of the sentence around it to be misheard again. That is a
 * far better second attempt than a repetition, and it is how people already
 * correct each other.
 */
export function repair(text) {
  const q = normalise(stripWake(String(text ?? "").trim()));

  const patterns = [
    /^(?:no|nope|wrong|not that)[,\s]+(?:i\s+)?(?:said|meant)\s+(.{2,60})$/,
    /^(?:i\s+)?(?:said|meant)\s+(.{2,60})$/,
    /^no[,\s]+(.{2,60})$/,
    /^not\s+\S.{0,38}?[,\s]+(.{2,60})$/,
  ];
  ...
  for (const pattern of patterns) {
    ...
    if (hit) {
      const fixed = hit[1].replace(/^(?:it was|it is|the)\s+/, "").trim();
...
export function topics(text) {
  ...
  /* ── NOTHING IS FETCHED ON THE STRENGTH OF ONE STRAY WORD ────────────
     A recogniser handed a cough, a chair scraping or half of somebody
     else's sentence returns a single plausible word. Guessing that it is a
     subject worth an encyclopaedia entry is how a board fills with rubbish
     nobody asked for. Names we keep a list of are exempt, because "Atiku"
     on its own is a real thing to say; everything else has to arrive in a
     sentence. */
  if (q.split(" ").filter(Boolean).length < 3) return [];
