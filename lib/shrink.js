 ...
 */
/* ── SIZED TO WHAT THE READER ACTUALLY ACCEPTS ────────────────────────────
   1,000,000 here matched a guard in lib/sheet-vision.js that was wrong about
   the reader's limit: it is 1.5 MB, not 1 MB. Those bytes are legibility —
   every figure on an EC8A is handwritten and the reader's whole job is a
   cramped corrected digit — so the picture is now allowed to keep them.

   Kept under the ceiling on both sides: this is the target the browser
   compresses towards, and sheet-vision refuses at 1.45 MB, so a photograph
   that lands slightly over target is still read rather than rejected at the
   door for the sake of a few kilobytes. */
export const RESULT_SHEET = { longestEdge: 2400, quality: 0.9, maxBytes: 1_400_000 };
