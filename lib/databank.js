...
async function markDone(id, why) {
  ...
  /* ── A SEPARATE STATEMENT, AND THAT IS THE POINT ───────────────────────
     Moving the body reads `body_elsewhere`, a column a migration adds — and
     in production this product applies migrations deliberately rather than on
     boot (see AUTO_MIGRATE in lib/db.js), so there is a window where the code
     is deployed and the column is not there yet.

     Folded into the update above, that window would mean every delivery
     failed to be recorded as made. A row never marked done is a row sent
     again, and again, for ever. Out here, the same missing column costs
     nothing but a body that stays where it is until the next delivery. */
  await archive(id);
}

/**
 * Move a delivered body into the second database.
 *
 * ══════════════════════════════════════════════════════════════════════════
 *  ONLY AFTER IT HAS LANDED, AND THAT IS THE WHOLE RULE
 *
 *  A body is the largest thing this product writes: a delivery carrying a
 *  photograph of Form EC8A is megabytes, and nothing ever removes one,
 *  because a delivery log is a record that something left this building and
 *  is kept for the eighteen months between a polling day and a tribunal.
 *  Measured on the working database, twenty-nine delivered rows were fourteen
 *  megabytes — more than a quarter of everything in it, and the count itself
 *  was two.
 *
 *  ── AND WHY NOT THE ONES STILL OWED ──────────────────────────────────────
 *  Because the drain claims a row and reads its body in one statement, on
 *  purpose, so that two instances cannot send the same return twice. Putting
 *  the body one network hop away would put a network inside the one path in
 *  this product that exists because the network cannot be trusted. A body
 *  still owed stays exactly where it is.
 *
 *  Once delivered, nothing reads it again except somebody asking, months
 *  later, what exactly was sent. That can afford to live at another address.
 * ══════════════════════════════════════════════════════════════════════════
 *
 * Never throws. A second database that is unreachable means a body stays
 * where it is and is moved the next time something is delivered — which is
 * the same state as not having one, and costs nothing but room.
 */
async function archive(id) {
  try {
    const { hasSecondDatabase, keepOutboxBody } = await import("./second-database.js");
    /* Asked before the row is read, so a deployment without a second database
       pays nothing at all for this — not one extra query per delivery. */
    if (!hasSecondDatabase()) return;

    const { sql } = await import("./sql.js");
    const rows = await sql`
      SELECT id, kind, external_id, body, body_elsewhere, delivered_at
        FROM dumpsite_outbox WHERE id = ${id}`;
    const row = rows?.[0];
    if (!row || row.body_elsewhere || row.body == null || row.body === "") return;

    const kept = await keepOutboxBody({
      id: row.id,
      kind: row.kind,
      externalId: row.external_id,
    ...
    /* Emptied, not nulled. The column is NOT NULL and a delivery log with
       holes in it is worse than one with a pointer: `body_elsewhere` is what
       says where to look, and it is set in the same statement that lets go. */
    await sql`UPDATE dumpsite_outbox SET body = '', body_elsewhere = true WHERE id = ${row.id}`;
  } catch {
    /* The body is still in the outbox, whole. Nothing is lost by failing. */
  }
}

/**
 * The body of a delivery already made, wherever it is held.
 *
 * Read by nothing on a polling night. This exists for the question that gets
 * asked afterwards — what exactly was sent, and when — and it answers it the
 * same way whether or not this deployment has a second database.
 */
export async function deliveredBody(id) {
  const { sql } = await import("./sql.js");
  const rows = await sql`SELECT body, body_elsewhere FROM dumpsite_outbox WHERE id = ${id}`;
  const row = rows?.[0];
  if (!row) return null;

  if (!row.body_elsewhere) return row.body;

  const { outboxBody } = await import("./second-database.js");
  const body = await outboxBody(id);
  if (body == null) {
    throw new Error(
      `The body of delivery ${id} is held in the second database, which could not be ` +
