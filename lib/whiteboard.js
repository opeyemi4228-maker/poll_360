...
export function buildCard(spec, context = {}) {
  ...
  /* ── WHAT CAME FROM OUTSIDE IS BUILT SEPARATELY ────────────────────────
     A reference has no place in this product's geography — there is no state
     to resolve, no level to apportion to, and nothing on it sums back to
     anything on the screen behind it. Running it through the same place
     resolution as a result card would give it a scope it does not have, and
     a card labelled "Nigeria" holding a paragraph about somewhere else is
     exactly the confusion this whole feature has to avoid. It carries what
     it was told, where it came from, and when it was read. */
  if (kind === "web") {
    counter += 1;
    return {
      ...
      ward: null,
      place: null,
      title: spec?.source ?? "From the web",
      subtitle: spec?.title ?? spec?.query ?? "Reference",
      text: spec?.extract ?? "",
      note: spec?.description ?? null,
      ambiguous: Boolean(spec?.ambiguous),
      image: spec?.image ?? null,
      href: spec?.href ?? null,
      source: spec?.source ?? "the web",
      readAt: spec?.at ?? Date.now(),
    };
  }
