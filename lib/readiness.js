import { reader, readsCertification, readsHandwriting } from "./sheet-vision.js";
...
export async function readiness() {
  ...
  /* ── AND CAN IT SEE WHO SIGNED THE SHEET? ────────────────────────────────
     Separate from the check above, because a deployment can pass that one and
     fail this one while looking perfectly healthy: a key is set, the reader
     runs, figures come back — and the presiding officer's signature and the
     official stamp are reported "unclear" on every sheet, forever, with
     nothing on any screen suggesting a cause.

     Only the model reader can answer these. A signature is a mark rather than
     a word and a stamp is an impression, so an optical reader has no text to
     return about either, however good it is at the figures. */
  const certification = readsCertification();

  if (!off) {
    checks.push({
      id: "sheet-certification",
      ok: certification,
      severity: "warning",
      title: certification
        ? "Signatures and stamps can be read from photographs"
        : "This reader cannot tell whether a sheet was signed or stamped",
      detail: certification
        ? "Each sheet is reported as signed, unsigned or not visible in the photograph, along with the presiding officer's name."
        : `A result sheet is evidence because a named officer signed and stamped it. The reader running here (${which}) returns the page as text, and a signature is a mark rather than a word — so every sheet will be recorded as "could not tell", which is honest but empty. The figures and the officer's name are still read.`,
      fix: certification
        ? null
        : "Set ANTHROPIC_API_KEY. It is the only configured reader that looks at the image rather than at text extracted from it.",
    });
  }
