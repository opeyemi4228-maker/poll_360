 ...
 * the strict one.
 *
 * ── IMPORTS NOTHING THAT IMPORTS ANYTHING ──────────────────────────────────
 ...
 * can run there and be tested here. The one import is lib/boot-script.js,
 * which holds the boot script's hashes as written-down constants and imports
 * nothing itself, for the same reason.
 */

import { BOOT_SCRIPT_HASHES } from "./boot-script.js";
...
export function contentSecurityPolicy({ nonce = null, development = false } = {}) {
  ...
  const script = nonce
    ? [
        ...
        "'strict-dynamic'",
        /* ── THE BOOT SCRIPT, BY HASH ─────────────────────────────────────
           The one inline script this product writes itself. It is constant,
           so it is allowed by what it says rather than by carrying this
           request's nonce — carrying the nonce is what broke hydration on
           every page, and routing it through the framework to get one is
           what stopped it running before the first paint. See
           lib/boot-script.js. */
        BOOT_SCRIPT_HASHES.production,
        BOOT_SCRIPT_HASHES.development,
