...
const TAB_WORD = {
  ...
  clearance: "live results",
  liveresults: "live results",
  analysis: "analysis",
  resultsintel: "analysis",
  dataviz: "analysis",
  trending: "analysis",
  listening: "claims",
  frames: "frames",
...
const SCREENS = {
  ...
  "live desk": "The live desk is where an update is released. Choose the place, from the whole country down to one polling unit, pick the card, write the words, and send it to an editor; a post marked to publish on clearing goes to every chosen platform the moment somebody else clears it. Beside it is everything waiting for an editor, of every kind, and underneath is the night on one clock: what the field reported, results arriving, and what the desk published, with a map showing where serious reports have had no update.",
  "on air": "On air is the output: the picture as it goes out, with the frame, the strap, the lower third and the ticker stacked at sixteen by nine; what is running now and next; and the shelf of cleared items, each one press from air. A cleared social post here is published everywhere it is aimed when it is taken to air.",
  "running order": "The running order is the night laid out in advance: programmes, result windows, breaking windows and streams, in time order. A slot is cleared like everything else, and a slot past its start that nobody took to air is shown as drifted rather than quietly dropped.",
  breaking: "The breaking desk shortens the typing, never the checking. One headline becomes a strap, a full frame and a post together, all of them drafted at once and all of them cleared by somebody who did not write them.",
  "live results": "Clearance is the control this whole arm exists for: raw, verified, cleared, on air, and every step between them is a person doing something. A place's figures may be read out only once an editor has cleared that place, and a return that arrives afterwards is not covered by it.",
  analysis: "Analysis is what the count says and how to say it on air: how much of it has been checked against its sheet, how the returns arrived, our count against any declaration, charts that print how many returns each bar rests on, and the trends worth two minutes of live analysis.",
  claims: "The claim desk ends in one of three answers, and unconfirmed is a real one rather than a failure. What this product is unusually able to do is hold a viral figure against the returns our own agents filed from the place it names.",
  frames: "Frames is the television bench: graphics built from templates and maps coloured by one question at a time, each filled from the live count with the coverage burnt in, and each cleared before it can be taken to air.",
  ticker: "The ticker desk writes the lines from the count rather than from memory. Every generated line carries the coverage behind it, because a share with no denominator is a different claim.",
  candidates: "Candidates holds each party's candidate for this contest: the name and a photograph, drawn on every result card from then on. A party with no candidate added is drawn with its logo instead.",
  delivery: "Delivery is what each platform did with what was sent: posted with a link, handed to the relay, failed with the platform's own reason, or not connected. Posts that did not reach everywhere they were aimed are listed first, each with a button to send again to just the ones that failed.",
  audit: "The audit trail is built from the desk's own stamps and the append-only log together, because the question somebody asks at two in the morning is how a particular thing got on air, and the answer needs both.",
  setup: "Setup is how each platform is reached tonight — directly, through the relay, or by hand — with a Check that asks each platform who we are without posting anything, and what this account is allowed to do on the desk.",
};
