...
export const GLOSSARY = [
  ...
  {
    term: "insecurity",
    aka: ["security pressure", "security", "insecurity", "violence"],
    real: false,
...
const TAB_WORD = {
  ...
  density: "clusters",
  watch: "coordinators",
  /* The room's own surfaces. Three of these are the words a merge left
     behind: "stream" and "alerts" both became Situations, and "sample"
     became a panel on the planning screen. They are kept pointing at what
     absorbed them rather than deleted, because somebody who has been saying
     "the incident feed" for a month must not find the phrase has stopped
     meaning anything. */
  stream: "situations",
  alerts: "situations",
  situations: "situations",
  sample: "planning",
  parties: "party ground",
  classify: "party ground",
  pulse: "overview",
  quality: "evidence",
  /* The broadcast arm. Its surface ids are what the shell switches on and are
     not always the word somebody would say — "centre" is the command centre,
     "multi" is multi-platform publishing — so they are mapped here for the
     same reason the room's are. The one deliberate near-miss is
     `liveresults`: the situation room already owns the word "results" for its
     map layer, and two screens answering to one word is how "what am I
     looking at" starts describing the wrong one. */
  centre: "command centre",
  playout: "master control",
  liveresults: "live results",
  mapstudio: "map studio",
  graphics: "graphics studio",
  socialdesk: "social command",
  content: "content studio",
  multi: "multi-platform",
  socialstats: "social analytics",
  feeds: "field feeds",
  resultsintel: "count quality",
  dataviz: "charts",
};
...
const SCREENS = {
  ...
  overview: "The overview is the one screen that answers whether this is working: how many returns are in against how many booths we have somebody at, what has moved in the last quarter of an hour, where to ring first, and what the checks are saying. Every figure on it opens the screen it came from.",
  coverage: "The booths screen is the funnel from a plan to a checked figure — assigned a booth, signed in, filed a return, verified — and, on the same screen, everything known about any one booth. It is measured against the roster and never against the registry, because a funnel that starts at a hundred and seventy six thousand booths reports catastrophe on a deployment doing exactly what it planned to.",
  situations: "Situations is one screen for anything going on: what this product noticed by itself, like a booth that has gone quiet, and what a person standing in a field rang in about. They were two tabs, and the wall between them ran through the middle of the only question the room asks — what is happening, and where. The two halves corroborate each other, and the map shows where they are.",
  "party ground": "Party ground is one party over one country, and you choose what the colour means. Share is how much of the vote that party took here, on a ramp of its own colour, so losing well and losing badly stop looking the same. Standing is what that amount is worth: held, genuinely in play, reachable, or gone. Same map, same picker, one press between them.",
  integrity: "Integrity asks three separate questions and keeps them apart: do the figures work against the register and their neighbours, does the sheet they were copied off agree with itself, and does our count match what was declared. None of it accuses anybody. Every line states arithmetic and stops there.",
  planning: "Planning is the deployment and the size it has to be, on one screen. Choose the states, local governments and wards to cover and it totals the polling units, the agents and the share of the register as you go; underneath, the sample arithmetic says whether the number you just drew can carry a projection at all. Margin of error, confidence and the design effect are all yours to set, because a sample size handed down as one number is a number nobody can argue with.",
  evidence: "The evidence screen counts what each return arrived carrying rather than what it said: the boxes at the head of the form, a photograph, whether that photograph was ever compared, a position, the agents who signed. Where one of those is missing the check that depends on it does not fail, it silently does not run, and that is what this screen exists to show.",
  ...
  /* ── THE BROADCAST ARM ────────────────────────────────────────────────
     Every one of these is written for somebody on their first shift at a
     desk during a live programme, which is the person most likely to ask
     what a screen is for and least able to go and find out. */
  "command centre": "The command centre answers one question: what should this team be saying right now. What is on air, what is waiting on an editor, how much of the count is in, and what has just come in from the field. Everything on it is one press from the screen it came from.",
  "master control": "Master control is the operator's queue. What is on air, what is cleared and ready to take, and what the running order said was supposed to be happening. Where those last two disagree it says so, because on a long night that gap is the thing worth knowing.",
  scheduler: "The scheduler is the night laid out in advance: programmes, result windows, breaking windows and streams. A slot is cleared like everything else, so the running order on the wall is the one the desk agreed to rather than the one somebody typed.",
  breaking: "The breaking desk shortens the typing, never the checking. One headline becomes a strap, a full frame and a post together, all of them drafted at once and cleared in one press by somebody who did not write them. There is no control anywhere on this desk that puts words on a screen with nobody else looking.",
  "control room": "The control room is the only screen that draws the output as one picture: the frame, the strap, the lower third and the ticker, stacked at the shape they go out at. Each layer can be individually correct and still contradict the others, and this is where that is visible.",
  "live results": "Live results is the control this whole arm exists for. Raw, verified, cleared, on air, and every step between them is a person doing something. A clearance is granted to a place at the coverage it had when it was granted, so a figure that has moved past what was passed says so rather than quietly widening.",
  "graphics studio": "The graphics bench fills the frame from the same rows the map is drawn from, at the moment it is asked, with the coverage burnt in. Templates the renderer cannot yet produce say so rather than drawing empty, and drafting against one still works: the item names the template and the place, and the audit records what went out.",
  "map studio": "The map studio colours the country by one of six questions, and says on the map which one. Leading party and reporting share look identical from across a studio and mean opposite things, so the caption travels with the picture. Grey is a place nobody has reported from, never a low figure.",
  ticker: "The ticker desk writes the lines from the count rather than from memory. Every generated line carries the coverage behind it, because a share with no denominator beside it is a claim the count does not support. Anything typed by hand goes through the same clearance.",
  streams: "Stream control holds the half of live video this product actually has: which streams are planned and where each one has got to. Bitrate, connection and viewer figures come from an encoder and a platform, neither of which is connected, so those meters are not drawn rather than drawn empty.",
  "social command": "The social desk composes, adapts and clears, and then hands over a file and a caption for a person to post. Nothing here publishes: a stored publishing token is the ability to say something in the organisation's name with nobody watching, which on this night is the largest risk the desk could carry.",
  "content studio": "The content studio writes one post and offers the caption the figures support, coverage and all. The format decides the shape; the platform decides how much of the caption survives the fold, which is where a coverage figure usually gets lost.",
  "multi-platform": "Multi-platform takes one cleared item and produces the several shapes the platforms need, adapting dimensions and trimming captions where each one truncates. Only cleared items can be adapted, because adapting a draft just produces seven drafts.",
  trending: "Trending shows the half of the question this product can answer honestly: where the count is moving, and which of those places the desk has broadcast nothing about. Platform trends need a read grant nobody holds, and a trend chart with nothing behind it changes what leads a bulletin.",
  listening: "Social listening keeps a public claim and a Poll360 figure visibly apart. One is what somebody posted; the other is what a named person filed from a named booth with a photographed sheet behind it. In one feed they acquire the same weight and get read out in one breath.",
  "social analytics": "Social analytics measures what this desk can measure: how much was written, how much survived an editor, and how long clearing took. Reach, views and engagement come back from a platform through a grant this product does not hold, so they are not drawn as zero.",
  video: "The video bench is a job sheet rather than an editor. It holds which packages exist, what each is for and where each has got to, because that is what a desk actually loses track of. The cutting happens in whatever the gallery already owns.",
  "field feeds": "Field feeds is what is arriving from the field and whether it carried a picture. The pictures themselves stay with the room running the count: a result sheet names a booth and an agent in one frame, and a broadcast account is deliberately not given the key.",
  reporters: "The reporter roster is the coordinator watch read as a newsroom list, because the person filing the return is the person a producer wants a two-way with. Every status is read off evidence rather than typed by anybody, and the column on the right says which evidence, so nobody confuses not there with not reachable.",
  charts: "The charts screen ranks places by one figure at a time and prints how many returns each bar rests on. A sixty-two per cent share off eleven booths and off nine hundred are the same bar and different claims, and the count beside it is what stops the wrong one being read out.",
  trends: "The trend board is material for live analysis: filing fastest, gone quiet, furthest through, closest, and highest turnout. All of it is about our returns rather than about the election, which is a distinction a presenter has to make out loud.",
  "count quality": "This screen answers how much weight the count will bear: how much has been checked against its sheet, how the returns arrived, and where the commission has declared, how our figures compare. A difference is coverage before it is dispute, and it becomes a story only when it survives our coverage reaching theirs.",
  claims: "The claim desk ends in one of three answers, and unconfirmed is a real one rather than a failure to produce something. What this product is unusually good at is the data check: a claim about a specific polling unit can be held against what that booth actually filed.",
  approvals: "The approvals queue is everything waiting on an editor, of every kind, in one place. Eleven separate approval screens would mean the quiet one never got looked at, and the quiet one is always the clearance for the figures.",
  audit: "The audit trail is built from the desk's own stamps and the append-only log together, because the question somebody asks at two in the morning is how a figure got on air. A draft can be discarded; nothing that has been through an editor ever is.",
  team: "The team screen maps a newsroom's dozen job titles onto the three grants this product actually enforces. The separation that protects the output is not between titles but between people: nobody clears their own work, checked on the item, and it holds for the administrator too.",
  platforms: "Platform connections says the same thing about every row, from one fact: nothing is connected. No publishing token is held anywhere in this product, so no screen can report a post as sent, which is exactly the failure a connected-looking dashboard produces on the night it matters.",
  health: "The health screen is measured from what the page already fetched rather than from a probe. A health screen that invents its own reassurance is the most dangerous screen in a product, because it is the one somebody checks before deciding not to investigate.",
};
    ...
    answer: (q, ents) => {
      ...
      if (term && !term.real && factor) {
        const value = {
          ...
          insecurity: `insecurity of ${factor.security} out of 100`,
