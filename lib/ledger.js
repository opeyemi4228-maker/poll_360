 ...
 * Change one figure in row 400 and every hash from 400 onward stops matching —
 * `verify()` finds it in one pass. Nobody, including whoever runs the database,
...
export const ledger = {
  ...
  append({ userId, kind, amount, note = null, actorId = null }) {
    ...
    const entry = {
      ...
      /* A stable, opaque reference derived from the account and the entry —
         enough to look a payment up and discuss it in a room, useless for
   ...
   * WITHDRAWAL_REQUESTED entry existed it *increased* the agent's balance —
   * the request to take money out paid them again. Anything not named here
