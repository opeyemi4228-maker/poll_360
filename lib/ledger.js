 ...
 */
/**
 * The timestamp exactly as it was hashed.
 *
 * ── WHY THIS FUNCTION HAS TO EXIST ─────────────────────────────────────────
 * An entry is hashed over an ISO string. sqlite stored text and gave back the
 * same text, so the two always matched. Postgres stores a timestamp and gives
 * back a Date, which stringifies as "Fri Aug 21 2026 04:12:33 GMT+0100" and
 * hashes to something else entirely. Every single entry then failed
 * verification, and the failure said "entry altered after it was written",
 * which is the most alarming thing this system can say and was, in this case,
 * entirely the storage layer's fault.
 *
 * Normalising here means the chain is hashed over the same characters
 * whichever engine is underneath, and a real tamper still fails.
 */
...
export const ledger = {
   ...
   */
  async append({ userId, kind, amount, note = null, actorId = null }) {
  ...
  async recent(take = 25) {
    const rows = await db.prepare("SELECT * FROM ledger ORDER BY seq DESC LIMIT ?").all(take);
    return rows.map(shape);
  },
   ...
   */
  async verify() {
    const rows = await db.prepare("SELECT * FROM ledger ORDER BY seq ASC").all();
