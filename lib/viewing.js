...
export const viewing = cache(async function viewing(pathname) {
  ...
  /* Resolved before the project is read, because the ground decides where an
     account lands the first time it signs in — see currentElection. A stated
     preference still wins; this only breaks the tie when there is none. */
  const project = await currentElection(resolved?.stateCode ?? null);
